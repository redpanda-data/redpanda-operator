...
func scriptParamsForLifecycle(state *RenderState, pool *redpandav1alpha2.NodePool) ScriptParams {
	return ScriptParams{
		...
		AdminHTTPProtocol: state.PoolSpec(pool).AdminInternalHTTPProtocol(),
		AdminAPIURLs:      state.AdminAPIURLs(pool),
	}
}
...
func scriptParamsFromState(state *RenderState, pool *redpandav1alpha2.NodePool) ScriptParams {
	p := ScriptParams{
		...
		KafkaPort:                   state.PoolSpec(pool).KafkaPort(),
		HTTPPort:                    state.PoolSpec(pool).HTTPPort(),
		RedpandaAtLeast22_3:         state.Spec().Image.AtLeast("22.3.0"),
		RackAwarenessEnabled:        state.PoolSpec(pool).RackAwareness.IsEnabled(),
		RackAwarenessNodeAnnotation: state.PoolSpec(pool).RackAwareness.GetNodeAnnotation(),
		AdminHTTPProtocol:           state.PoolSpec(pool).AdminInternalHTTPProtocol(),
		AdminAPIURLs:                state.AdminAPIURLs(pool),
		RPCPort:                     state.PoolSpec(pool).RPCPort(),
	}
	...
	if l := state.PoolSpec(pool).Listeners; l != nil && l.HTTP != nil {
...
func (r *RenderState) adminTLSCurlFlags(pool *redpandav1alpha2.NodePool) string {
	...
	certName := r.PoolSpec(pool).Listeners.AdminCertName()
	if certName == "" {
		return ""
	}
	...
	return fmt.Sprintf("--cacert %s", r.PoolSpec(pool).TLS.CertServerCAPath(certName))
}
