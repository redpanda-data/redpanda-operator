...
const (
	...
	// maxDebounceWindow caps how long a sustained event stream can defer a
	// render. Without it, events arriving more often than debounceWindow
	// would keep pushing the deadline out indefinitely and the render would
	// only fire via the resync ticker. ConfigMap swaps are short bursts so
	// this is belt-and-suspenders, but it bounds worst-case staleness.
...
func WatchAndRender(ctx context.Context, log logr.Logger, sourceDir string, resync time.Duration, render func(context.Context) error) error {
	...
	// burstStart marks the first event of the current debounce burst, so a
	// sustained stream can be capped at maxDebounceWindow (see below).
	...
	for {
		...
		case <-debounceC:
			debounceC = nil
			debounce = nil // next event starts a fresh burst
			doRender()
		case _, ok := <-watcher.Events:
			...
			case time.Since(burstStart) >= maxDebounceWindow:
				// Burst has run longer than the cap: stop pushing the
				// deadline out and let the pending timer fire.
			default:
				if !debounce.Stop() {
					select {
					case <-debounce.C:
...
// NeedLeaderElection makes the watcher run in every sidecar, not just the
// elected leader. It refreshes the *local* pod's rpk configuration, so — like
// the per-pod broker probe (operator/internal/probes) — it must run on every
// broker pod; otherwise only the single elected sidecar would converge and
// every other pod would keep a stale rpk stanza. (K8S-755)
func (w *Watcher) NeedLeaderElection() bool {
	return false
}

var (
	_ manager.Runnable               = (*Watcher)(nil)
	_ manager.LeaderElectionRunnable = (*Watcher)(nil)
)
