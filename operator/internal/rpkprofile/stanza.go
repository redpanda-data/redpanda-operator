...
//
// A source whose `rpk` key is missing, null, or an empty mapping is a no-op
// rather than an error: it must never clobber a working destination during a
// partially-propagated mount update. The destination is replaced atomically
// (temp file + rename) since in-pod rpk may read it concurrently.
func SyncRPKStanza(sourcePath, destPath string) error {
	...
	if !ok || rpkStanza == nil {
		// Missing or explicitly-null rpk: the source is incomplete or
		// mid-render. Never overwrite a working destination with it.
		return nil
	}
	// A fully-rendered source always carries a populated rpk stanza, so an
	// empty mapping signals a partial read — treat it as a no-op too, rather
	// than wiping the destination's broker/admin/schema-registry lists.
	if m, isMap := rpkStanza.(map[string]any); isMap && len(m) == 0 {
		return nil
	}
...
//
// Durability (fsync of the temp file / parent dir) is intentionally out of
// scope: both destinations live on a pod-local emptyDir that is fully
// re-rendered at the next pod start, so a crash before the rename reaches
// stable storage self-heals. "Atomic" here means atomic with respect to
// concurrent readers, not crash-durable.
