...
// TestSyncRPKStanzaNoopWithNullOrEmptyRPK ensures a present-but-empty rpk
// stanza (null or {}) — which a partially-propagated mount can produce — never
// clobbers a working destination's broker/admin/schema-registry lists.
func TestSyncRPKStanzaNoopWithNullOrEmptyRPK(t *testing.T) {
	original := "rpk:\n  kafka_api:\n    brokers:\n      - keep-me:9093\n"
	for name, srcContent := range map[string]string{
		"null rpk":  "rpk:\n",
		"empty rpk": "rpk: {}\n",
	} {
		t.Run(name, func(t *testing.T) {
			...
			require.NoError(t, os.WriteFile(source, []byte(srcContent), 0o644))
			require.NoError(t, os.WriteFile(dest, []byte(original), 0o644))
			...
			buf, err := os.ReadFile(dest)
			require.NoError(t, err)
			assert.Equal(t, original, string(buf), "empty/null rpk source must not touch the destination")
		})
	}
}

// TestWatcherDoesNotNeedLeaderElection guards the K8S-755 correctness
// requirement that the v2 sidecar watcher refreshes the *local* pod's rpk
// config on every broker pod — not only the elected leader. Registered on a
// leader-electing manager, a Runnable that omits this would silently default
// into the leader-election group and converge a single pod.
func TestWatcherDoesNotNeedLeaderElection(t *testing.T) {
	assert.False(t, (&Watcher{}).NeedLeaderElection(),
		"watcher must run in every sidecar, not just the elected leader")
}
