...
func TestStretchClusterResourceClient(t *testing.T) {
	...
	resourceClient := NewResourceClient(manager, StretchClusterResourceManagers(redpandaImage, sidecarImage, cloudSecrets))
	...
	for _, file := range casesArchive.Files {
		t.Run(file.Name, func(t *testing.T) {
			...
			cluster := NewStretchClusterWithPools(stretchCluster, pools...)

			ownerLabels := resourceClient.ownershipResolver.GetOwnerLabels(cluster)

			require.NotEmpty(t, ownerLabels)

			state, err := resourceClient.nodePoolRenderer.(*StretchNodePoolRenderer).convertToRender(cluster, "local")
			require.NoError(t, err)
			...
			sets, err := resourceClient.nodePoolRenderer.Render(ctx, cluster, "local")
			require.NoError(t, err)
			...
			resources, err := resourceClient.simpleResourceRenderer.Render(ctx, cluster, "local")
			require.NoError(t, err)
