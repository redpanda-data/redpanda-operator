...
// installBrokerCRD installs the real Broker CRD into the test API server.
...
func createBrokerPod(ctx context.Context, t *testing.T, cl client.Client, cluster *MockCluster, name, podHash string, ready bool) {
	t.Helper()
	...
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: metav1.NamespaceDefault,
			// Broker pods carry the owner labels in production (stamped via
			// the pod template); fetchBrokerBackedPools lists pods by them.
			Labels: map[string]string{
				"cluster-name": cluster.Name,
			},
			Annotations: map[string]string{
...
func TestClientFetchBrokerBackedPools(t *testing.T) {
	...
	tt.Run(parentCtx, t, "broker-backed-pools", func(t *testing.T, instances *clientTestInstances, cluster *MockCluster) {
		...
		// "sts-backed" has a live StatefulSet with a shadow Broker; the STS
		// view must stay authoritative.
		...
		createBrokerPod(ctx, t, instances.k8sClient, cluster, "rp-0", "hash-0", true)
		createBrokerPod(ctx, t, instances.k8sClient, cluster, "rp-1", "stale-hash", false)
		createBrokerPod(ctx, t, instances.k8sClient, cluster, "rp-2", "hash-2", true)
