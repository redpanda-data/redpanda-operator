...
// RedpandaBrokerPoolReconciler reconciles a RedpandaBrokerPool object. This reconciler in particular should only update status
// fields and finalizers on the RedpandaBrokerPool objects; rendering of broker pools takes place within the MulticlusterReconciler.
type RedpandaBrokerPoolReconciler struct {
...
// SetupRedpandaBrokerPoolWithMultiClusterManager registers the RedpandaBrokerPoolReconciler with the multicluster manager.
func SetupRedpandaBrokerPoolWithMultiClusterManager(mgr multicluster.Manager) error {
	mgr.GetLogger().WithName("SetupRedpandaBrokerPoolWithMultiClusterManager").Info("registering RedpandaBrokerPool controller", "knownClusters", createCanonicalClusterNameList(mgr))
	return mcbuilder.ControllerManagedBy(mgr).
		...
		Watches(&redpandav1alpha2.StretchCluster{}, func(_ string, _ cluster.Cluster) mchandler.EventHandler {
			return mchandler.TypedEnqueueRequestsFromMapFuncWithClusterPreservation(func(ctx context.Context, object client.Object) []mcreconcile.Request {
				l := log.FromContext(ctx).WithName("RedpandaBrokerPoolReconciler.StretchClusterWatch").V(log.TraceLevel)
				...
				l.Info("enqueuing RedpandaBrokerPool reconcile requests", "requests", reqs)
				return reqs
			})
		}).
		Complete(
			observability.Wrap[mcreconcile.Request](
				&RedpandaBrokerPoolReconciler{Manager: mgr},
				"RedpandaBrokerPool",
				periodicRequeue,
			),
		)
...
func (r *RedpandaBrokerPoolReconciler) Reconcile(ctx context.Context, req mcreconcile.Request) (result ctrl.Result, err error) {
	...
	if err := k8sClient.List(ctx, &statefulSets, client.MatchingLabels{
		...
		rendermulticluster.BrokerPoolLabelName: pool.Name,
	}); err != nil {
	...
	if sts != nil {
		...
		if stsLabels != nil {
			generationString := stsLabels[rendermulticluster.BrokerPoolLabelGeneration]
			if generationString != "" {
	...
	if err := r.getStretchCluster(ctx, req, pool, k8sClient); err != nil {
...
func (r *RedpandaBrokerPoolReconciler) getStretchCluster(
	ctx context.Context,
