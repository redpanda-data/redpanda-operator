...
// last_success_timestamp_seconds). One wrap call per controller; the
// controller's SetupWithManager passes the wrapped reconciler to
// ControllerManagedBy's Complete or Build.
//
...
// state. Pass 0 for controllers that don't use the periodic-requeue
// pattern; under that setting only Result{} counts as steady state.
//
...
// record updates the wrapper's Group B counters / gauges from one
