...
func NewMuliticlusterFactory(mgr multicluster.Manager, expander *pkgsecrets.CloudExpander) *Factory {
	return &Factory{
		mgr:                mgr,
		fs:                 afero.NewOsFs(),
		secretExpander:     expander,
...
func (c *Factory) getClient(ctx context.Context, clusterName string) (client.Client, error) {
	if c.mgr == nil && clusterName != mcmanager.LocalCluster {
		return nil, errors.New("cannot specify a cluster name if not initialized with a multicluster manager")
	}
...
func (c *Factory) getConfig(ctx context.Context, clusterName string) (*rest.Config, error) {
	...
	return nil, errors.New("could not fetch rest config")
}
