...
// DefaultKubeClusterDomain is the Kubelet cluster domain assumed when none is
// configured via WithClusterDomain. Named apart from
// redpandav1alpha2.DefaultClusterDomain, which is the same domain carrying the
// trailing dot.
const DefaultKubeClusterDomain = "cluster.local"
...
type Factory struct {
	...
	// of V1 clusters. Empty means DefaultKubeClusterDomain.
...
// domain returns the configured cluster domain or DefaultKubeClusterDomain.
func (c *Factory) domain() string {
	if c.clusterDomain == "" {
		return DefaultKubeClusterDomain
	}
