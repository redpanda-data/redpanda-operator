...
func (c *Factory) remoteClusterTLSSettingsForSpec(ctx context.Context, namespace string, spec *ir.CommonTLS) (*shadow.TLSSettings, error) {
	tls, err := spec.Load(ctx, c.Client)
	if err != nil {
		return nil, err
	}

	return &shadow.TLSSettings{
		CA:   tls.CA,
		Cert: tls.Cert,
		Key:  tls.Key,
	}, nil
...
func (c *Factory) remoteClusterAuthSettingsForSpec(ctx context.Context, namespace string, spec *ir.KafkaAPISpec) (*shadow.AuthenticationSettings, error) {
	switch spec.SASL.Mechanism {
	case config.SASLMechanismPlain, config.SASLMechanismScramSHA256, config.SASLMechanismScramSHA512:
		p, err := spec.SASL.Load(ctx, c.Client)
		if err != nil {
		...
		return &shadow.AuthenticationSettings{
			Username:  p.Username,
			Password:  p.Password,
			Mechanism: redpandav1alpha2.SASLMechanism(p.Mechanism),
		}, nil
	}
