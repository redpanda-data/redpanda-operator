	...
	for key, value := range props {
		if nodeProp := isKnownNodeProperty(key); !nodeProp && strings.HasPrefix(key, redpandaPropertyPrefix) {
			...
			if targetConfig.BootstrapConfiguration == nil {
				targetConfig.BootstrapConfiguration = make(map[string]vectorizedv1alpha1.ClusterConfigCRDValue)
			}
			targetConfig.BootstrapConfiguration[newKey] = vectorizedv1alpha1.ClusterConfigCRDValue{
				Representation: ptr.To(vectorizedv1alpha1.YAMLRepresentation(value)),
			}
		} else {
			remaining[key] = value
		}
