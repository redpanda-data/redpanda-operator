...
func RenderHeadlessService(
	_ context.Context,
	cluster *vectorizedv1alpha1.Cluster,
) (k8sclient.Object, error) {
	headlessPorts := collectHeadlessPorts(cluster)
...
// collectHeadlessPorts collects all internal ports for the headless service
func collectHeadlessPorts(cluster *vectorizedv1alpha1.Cluster) []NamedServicePort {
...
func getHeadlessServiceAnnotations(cluster *vectorizedv1alpha1.Cluster) map[string]string {
	externalListener := cluster.ExternalListener()
	if externalListener == nil || externalListener.External.Subdomain == "" {
		return nil
	}
