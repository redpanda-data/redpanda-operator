...
func CreateConfiguration(
	...
	profile.KafkaAPI.Brokers = addresses
	if tls := tlsConfigProvider.KafkaClientBrokerTLS(mountPoints); tls != nil {
		profile.KafkaAPI.TLS = &config.TLS{
			KeyFile:            tls.KeyFile,
	...
	profile.AdminAPI.Addresses = addresses
	if tls := tlsConfigProvider.AdminAPIClientTLS(mountPoints); tls != nil {
		profile.AdminAPI.TLS = tls
	}

	profile.SR.Addresses = addresses
	if tls := tlsConfigProvider.SchemaRegistryClientTLS(mountPoints); tls != nil {
		profile.SR.TLS = tls
	}

	cfg.RPK.Profiles = []config.RpkProfile{profile}
