...
func CreateConfiguration(
	...
	if pandaCluster.Spec.EnableSASL {
		if err := clusterconfiguration.SetValue(cfg.Cluster, "enable_sasl", true); err != nil {
	...
	if pandaCluster.Spec.KafkaEnableAuthorization != nil && *pandaCluster.Spec.KafkaEnableAuthorization {
		if err := clusterconfiguration.SetValue(cfg.Cluster, "kafka_enable_authorization", true); err != nil {
	...
	if partitions != 0 {
		if err := clusterconfiguration.SetValue(cfg.Cluster, "group_topic_partitions", partitions); err != nil {
			return nil, fmt.Errorf("setting group_topic_partitions: %w", err)
		}
	}

	if err := clusterconfiguration.SetValue(cfg.Cluster, "auto_create_topics_enabled", pandaCluster.Spec.Configuration.AutoCreateTopics); err != nil {
		return nil, fmt.Errorf("setting auto_create_topics_enabled: %w", err)
	}

	intervalSec := 60 * 30 // 60s * 30 = 30 minutes
	if err := clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_segment_max_upload_interval_sec", intervalSec); err != nil {
		return nil, fmt.Errorf("setting cloud_storage_segment_max_upload_interval_sec: %w", err)
	}

	if err := clusterconfiguration.SetValue(cfg.Cluster, "log_segment_size", logSegmentSize); err != nil {
	...
	err = prepareSchemaRegistryClient(ctx, cfg, serviceFQDN, tlsConfigProvider, pandaCluster, schemaRegistrySASLUser, mountPoints)
	if err != nil {
		return nil, fmt.Errorf("preparing schemaRegistry client: %w", err)
	}

	if err := clusterconfiguration.SetValue(cfg.Cluster, "enable_rack_awareness", true); err != nil {
...
func prepareCloudStorage(
	...
	if pandaCluster.Spec.CloudStorage.AccessKey != "" {
		_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_access_key", pandaCluster.Spec.CloudStorage.AccessKey)
	}
	...
	if pandaCluster.Spec.CloudStorage.CredentialsSource != "" {
		_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_credentials_source", string(pandaCluster.Spec.CloudStorage.CredentialsSource))
	}

	_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_enabled", pandaCluster.Spec.CloudStorage.Enabled)
	_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_region", pandaCluster.Spec.CloudStorage.Region)
	_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_bucket", pandaCluster.Spec.CloudStorage.Bucket)
	_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_disable_tls", pandaCluster.Spec.CloudStorage.DisableTLS)

	interval := pandaCluster.Spec.CloudStorage.ReconcilicationIntervalMs
	if interval != 0 {
		_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_reconciliation_interval_ms", interval)
	}
	maxCon := pandaCluster.Spec.CloudStorage.MaxConnections
	if maxCon != 0 {
		_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_max_connections", maxCon)
	}
	apiEndpoint := pandaCluster.Spec.CloudStorage.APIEndpoint
	if apiEndpoint != "" {
		_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_api_endpoint", apiEndpoint)
	}
	endpointPort := pandaCluster.Spec.CloudStorage.APIEndpointPort
	if endpointPort != 0 {
		_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_api_endpoint_port", endpointPort)
	}
	trustfile := pandaCluster.Spec.CloudStorage.Trustfile
	if trustfile != "" {
		_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_trust_file", trustfile)
	}
	...
	if pandaCluster.Spec.CloudStorage.CacheStorage != nil && pandaCluster.Spec.CloudStorage.CacheStorage.Capacity.Value() > 0 && !cloudStoragePercentageConfigured {
		...
		_ = clusterconfiguration.SetValue(cfg.Cluster, "cloud_storage_cache_size", pandaCluster.Spec.CloudStorage.CacheStorage.Capacity.Value())
	}
...
func preparePandaproxyClient(
	...
	cfg.Node.AddFixup("pandaproxy_client.scram_username", `envString("PANDAPROXY_CLIENT_USERNAME")`)
	cfg.Cluster.AddFixup(superusersConfigurationKey, `appendYamlStringArray(envString("PANDAPROXY_CLIENT_USERNAME"))`)
	_ = cfg.EnsureInitEnv(corev1.EnvVar{
	...
	cfg.Node.AddFixup("pandaproxy_client.scram_password", `envString("PANDAPROXY_CLIENT_PASSWORD")`)
	return cfg.Error()
}

func prepareSchemaRegistryClient(
	ctx context.Context, cfg *clusterconfiguration.CombinedCfg,
	serviceFQDN string,
	tlsConfigProvider resourcetypes.BrokerTLSConfigProvider,
	pandaCluster *vectorizedv1alpha1.Cluster,
	schemaRegistrySASLUser types.NamespacedName,
	...
	cfg.Node.AddFixup("schema_registry_client.scram_username", `envString("SCHEMA_REGISTRY_CLIENT_USERNAME")`)
	cfg.Cluster.AddFixup(superusersConfigurationKey, `appendYamlStringArray(envString("SCHEMA_REGISTRY_CLIENT_USERNAME"))`)
	_ = cfg.EnsureInitEnv(corev1.EnvVar{
		Name: "PANDAPROXY_CLIENT_PASSWORD",
		ValueFrom: &corev1.EnvVarSource{
			SecretKeyRef: &corev1.SecretKeySelector{
	...
	cfg.Node.AddFixup("schema_registry_client.scram_password", `envString("SCHEMA_REGISTRY_CLIENT_PASSWORD")`)
	return cfg.Error()
}
