...
until [ "$retries" -lt 0 ]; do
  echo "Fetching configuration from $NAMESPACE/multi-external-listeners-0"
  actual=$(kubectl -n "$NAMESPACE" exec multi-external-listeners-0 -- cat /etc/redpanda/redpanda.yaml)
