...
func (o *MulticlusterOptions) BindFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVar(&o.AllowPVRebinding, "allow-pv-rebinding", false, "DEPRECATED and inert for PV writes: the PVCUnbinder now always reserves a freed PV for its recreated claim (ClaimRef UID cleared, namespace/name kept) while the PV's node exists, so the disk is reused without the cross-broker swap risk this flag carried. Setting it only forces the most conservative pvc-rebinding gate behavior (no stuck-claim exemption).")
	_ = cmd.Flags().MarkDeprecated("allow-pv-rebinding", "the PVCUnbinder now reserves freed PVs for their recreated claims unconditionally; this flag no longer clears ClaimRefs and only disables the pvc-rebinding gate's stuck-claim exemption")
