...
type MulticlusterOptions struct {
	...
	// Schema Registry store having caught up on _schemas; mirrors the `run`
	// command's flag of the same name.
...
func (o *MulticlusterOptions) BindFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVar(&o.AllowPVRebinding, "allow-pv-rebinding", false, "DEPRECATED and inert for PV writes: the PVCUnbinder now reserves a freed PV for its recreated claim (ClaimRef UID cleared, namespace/name kept) whenever the PV's pinned node could still host the pod, so the disk is reused without the cross-broker swap risk this flag carried; a PV whose node is gone (or permanently blocked by the pod's own hard anti-affinity) is left Released instead. Setting the flag only forces the most conservative pvc-rebinding gate behavior (no stuck-claim exemption).")
	_ = cmd.Flags().MarkDeprecated("allow-pv-rebinding", "the PVCUnbinder now reserves freed PVs for their recreated claims regardless of this flag (while the PV's pinned node could still host the pod; otherwise the PV is left Released); the flag no longer clears ClaimRefs and only disables the pvc-rebinding gate's stuck-claim exemption")
	...
	cmd.Flags().BoolVar(&o.WaitForSchemaRegistrySync, "wait-for-schema-registry-sync", true, "During a rolling restart, wait for each broker's Schema Registry store to report caught up on _schemas (GET /status/ready on the SR listener) before the next broker is rolled, so overlapping SR replay windows can't leave the cluster without a consistent Schema Registry endpoint mid-upgrade. Skipped when broker pools disable or disagree on their Schema Registry listener config, since the shared probe assumes a homogeneous listener. Set to false to roll without waiting on Schema Registry.")
