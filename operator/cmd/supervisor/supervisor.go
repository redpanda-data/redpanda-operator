...
The default retry parameters are tuned for a a long running process, such as the sidecar subcommand.`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for {
				...
				if delay > 0 {
					logger.Info("backing off retries", "delay", delay.String())
				}
...
func (r *Runner) Run(
	...
	logger.Info("starting process", "command", r.Command)
	...
	if err := subprocess.Start(); err != nil {
		logger.Error(err, "failed to start process", "command", r.Command)
	...
	for {
		select {
		case err := <-doneCh:
			logger.Error(err, "process exited")
		...
		case sig := <-r.Signals:
			...
			if err := subprocess.Process.Signal(sig); err != nil {
				logger.Error(err, "failed to forward signal to subprocess", "signal", sig)
			}
			continue
		...
		case <-ctx.Done():
			...
			select {
			case <-doneCh:
				logger.Info("process gracefully terminated")
