...
func TestDefluxedMinimumVersion(t *testing.T) {
	...
	require.Containsf(
		...
		"'useFlux' should reference the version %q of the go.mod installed redpanda chart. Do you need to run 'task k8s:generate' or update the comment?",
		redpanda.Chart.Metadata().Version,
	)
}
...
func TestHelmValuesCompat(t *testing.T) {
	cfg := rapid.MakeConfig{
		Types: map[reflect.Type]*rapid.Generator[any]{
			...
			reflect.TypeFor[*applycorev1.PodSpecApplyConfiguration](): rapid.Just[any](nil), // PodSpecApplyConfiguration's serialization intentionally diverges from PodSpecApplyConfiguration's so we can leverage builtin types and their validation.
