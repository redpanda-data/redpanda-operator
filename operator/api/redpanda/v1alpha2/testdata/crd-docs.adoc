...
| *`replicas`* __integer__ | Replicas is the number of pipeline replicas to run. + | 1 | Minimum: 0 +
Optional: \{} +
...
| *`paused`* __boolean__ | Paused stops the pipeline by scaling replicas to zero when set to true. + |  | Optional: \{} +
...
| *`replicas`* __integer__ | Replicas is the number of desired replicas. + |  | Optional: \{} +
