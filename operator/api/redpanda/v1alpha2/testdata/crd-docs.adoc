...
It is used in constructing the client created to configure a cluster. + |  | 
| *`principals`* __string array__ | Principals lists Redpanda user principals directly assigned to this role. +
...
Supported types: currently only User (either explicit 'User:' prefix or no prefix). + |  | MaxItems: 1024 +
...
to be cleaned up. + |  | 
| *`principals`* __string array__ | Principals is the last successfully synced list of principals for this role. +
It mirrors the normalized spec.principals currently supported; RoleBinding +
aggregation is not yet implemented. +
Pattern validation ensures entries are in normalized "Type:Name" format. + |  | MaxItems: 1024 +
