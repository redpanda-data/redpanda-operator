...
[id="{anchor_prefix}-github-com-redpanda-data-redpanda-operator-operator-api-redpanda-v1alpha2-brokerdecommissioner"]
...
| *`decommissionAfter`* __string__ | Specifies how long to wait after a broker's Pod is removed before +
decommissioning it. Accepts a Go duration string, for example `60s`. + |  | 
| *`decommissionRequeueTimeout`* __string__ | Specifies how long to wait before re-checking a broker that is being +
...
[id="{anchor_prefix}-github-com-redpanda-data-redpanda-operator-operator-api-redpanda-v1alpha2-pvcunbinder"]
==== PVCUnbinder
...
| Field | Description | Default | Validation
| *`enabled`* __boolean__ | Specifies whether the sidecar is enabled. + |  | 
| *`unbindAfter`* __string__ | Specifies how long a broker Pod must be stuck `Pending` (due to a volume +
...
| *`pvcUnbinder`* __xref:{anchor_prefix}-github-com-redpanda-data-redpanda-operator-operator-api-redpanda-v1alpha2-pvcunbinder[$$PVCUnbinder$$]__ | Configures the `pvc-unbinder` sidecar, which unbinds the PersistentVolumeClaim +
...
| *`brokerDecommissioner`* __xref:{anchor_prefix}-github-com-redpanda-data-redpanda-operator-operator-api-redpanda-v1alpha2-brokerdecommissioner[$$BrokerDecommissioner$$]__ | Configures the `broker-decommissioner` sidecar, which decommissions brokers +
