...
func TestConversion(t *testing.T) {
	...
	yamlRepr := rapid.Custom(func(t *rapid.T) redpandav1alpha3.YAMLRepresentation {
		// TODO some fun issues pop up when allowing maps or rapid.Int here.
		value := rapid.OneOf(
			rapid.Just[any](nil),
			rapid.Float32().AsAny(),
			rapid.Int32().AsAny(),
	...
	cfg := rapid.MakeConfig{
		// Kinds: rapidutil.KubernetesTypes.Kinds,
		Kinds: extend(rapidutil.KubernetesTypes.Kinds, map[reflect.Kind]*rapid.Generator[any]{
			// Could also make this a field level override for Port?
