...
func createVClusters(ctx context.Context, t framework.TestingT, clusters int32) []*vclusterNode {
	...
	for i := range clusters {
		...
		go func(i int) {
			...
			vClusterValues := vcluster.DefaultValues + networkingValues(i, int(clusters))
...
// replicated into it.
func networkingValues(index, total int) string {
	var entries []string
	for j := range total {
		if j == index {
			continue
...
func deployOperators(ctx context.Context, t framework.TestingT, vclusters []*vclusterNode, namespace, redpandaLicense string, peers []any) {
	...
	for _, cluster := range vclusters {
		...
		issuerRetry := wait.Backoff{
			Steps:    100,
			Duration: 1 * time.Second,
			Factor:   1.0,
			Jitter:   0.1,
		}
		isWebhookError := func(err error) bool {
			return err != nil && strings.Contains(err.Error(), "webhook")
		}
		require.NoError(t, retry.OnError(issuerRetry, isWebhookError, func() error {
