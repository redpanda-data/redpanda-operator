...
# tag::manage-authz-only-roles[]
    # In this example manifest, a role called "travis" is created in a cluster called "roles".
...
# end::manage-authz-only-roles[]
    """
    And role "travis" is successfully synced
    And I delete the CRD role "travis"
    Then there should be no role "travis" in cluster "roles"