...
Feature: Broker CRD with V2 Redpanda
  ...
  Scenario: Redpanda with broker-cr annotation creates Broker CRs instead of a StatefulSet
    ...
    And all Broker CRs for cluster "broker-v2" should be Running
    And cluster "broker-v2" admin API should show 3 brokers
    # A StatefulSet can only ever remove the highest ordinal; with Broker
    # CRs any broker can be replaced in place.
    ...
    And all Broker CRs for cluster "broker-v2" should be Running
    And cluster "broker-v2" admin API should show 3 brokers
    # Roll back a broker-BORN cluster (no migration backup exists). The final
    # roll is the load-bearing assertion: an adopted pod without revision
    # bookkeeping is silently skipped by every future template change.
