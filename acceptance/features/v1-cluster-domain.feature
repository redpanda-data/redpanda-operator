...
Feature: V1 clusters on a non-default cluster domain
  ...
  # and installs its own operator through the chart's clusterDomain value,
  # which is the supported way to set it. Every V2 CR that reaches a V1
  # Cluster over the admin API goes through those builders, so a User CR
  # reaching Synced proves the client dialed a resolvable hostname and
  ...
  Scenario: User CRs sync against a V1 cluster with a TLS admin API on a custom domain
    ...
    # Role CRs exercise both V1 builders at once: the role lands over the
    # admin API and the ACL over the Kafka client, so the two have to agree on
    # the domain.
    When I apply Kubernetes manifest:
    """
    ---
