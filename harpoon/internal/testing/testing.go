...
func (t *TestingT) VCluster(ctx context.Context, clusterDomain string) string {
	...
	t.Cleanup(func(ctx context.Context) {
		// Switch back before tearing down: require.NoError below calls
		// FailNow, which would abandon the rest of this closure and leave
		// every handle pointing at a vcluster whose port-forward is already
		// cancelled. Later hooks would then fail with connection errors that
		// bury whatever actually went wrong here.
		proxyCancel()
