...
func pooledRedpandaConfigMaps(dot *helmette.Dot, pools []*redpandav1alpha3.NodePool) []*corev1.ConfigMap {
	cms := []*corev1.ConfigMap{}
	for _, pool := range pools {
		cms = append(cms, pooledRedpandaConfigMap(dot, pool, pools))
	}
	return cms
}

func pooledRedpandaConfigMap(dot *helmette.Dot, pool *redpandav1alpha3.NodePool, pools []*redpandav1alpha3.NodePool) *corev1.ConfigMap {
	bootstrap, fixups := BootstrapFile(dot)

	// Per-pool Redpanda configuration - ugly hack requires remarshaling for now
	redpandaTemplate := RedpandaConfigFile(dot, pools, true /* includeSeedServer */)
	template := helmette.FromYaml[map[string]any](redpandaTemplate)
	poolExtra, poolFixes, _ := translateSource(pool.Spec.BrokerTemplate.NodeConfig)
	template = helmette.Merge(template, poolExtra)
	redpandaTemplate = helmette.ToYaml(template)
	poolFixups := helmette.ToYaml(poolFixes)
	if len(poolFixes) == 0 {
		poolFixups = `[]`
	}
	...
	return &corev1.ConfigMap{
		...
		Data: map[string]string{
			...
			clusterconfiguration.RedpandaYamlTemplateFile: redpandaTemplate,
			clusterconfiguration.RedpandaYamlFixupFile:    poolFixups,
		},
	}
}
...
func renderPool(dot *helmette.Dot, pool *redpandav1alpha3.NodePool, pools []*redpandav1alpha3.NodePool) *appsv1.StatefulSet {
	...
	ss := &appsv1.StatefulSet{
		...
		Spec: appsv1.StatefulSetSpec{
			...
			// TODO: filter out duplicates below
			VolumeClaimTemplates: pool.Spec.BrokerTemplate.VolumeClaimTemplates,
		},
	}
...
func poolDefaultInitContainers(dot *helmette.Dot, pool *redpandav1alpha3.NodePool) []corev1.Container {
...

	{
		...
		_, _, nodeEnv := translateSource(pool.Spec.BrokerTemplate.NodeConfig)
		...
		containers = append(containers, corev1.Container{
			...
			Env: append(append(env, additionalEnv...), nodeEnv...),
		...
		},
		{
			...
			VolumeSource: corev1.VolumeSource{
				ConfigMap: &corev1.ConfigMapVolumeSource{
					LocalObjectReference: corev1.LocalObjectReference{Name: fmt.Sprintf("%s-%s", fullname, pool.Name)},
				},
			},
		},
		{
...
func NodePoolLabelsSelector(dot *helmette.Dot, pool *redpandav1alpha3.NodePool) map[string]string {
	prefix := strings.TrimSuffix(helmette.Trunc(51, fmt.Sprintf("%s-%s", Fullname(dot), pool.Name)), "-")
	component := fmt.Sprintf("%s-statefulset", prefix)
...
func ClusterPodLabelsSelector(dot *helmette.Dot) map[string]string {
	return map[string]string{
		"app.kubernetes.io/instance":  dot.Release.Name,
		"app.kubernetes.io/name":      Name(dot),
		"cluster.redpanda.com/broker": "true",
...
func translateSource(source map[string]redpandav1alpha3.ValueSource) (map[string]any, []clusterconfiguration.Fixup, []corev1.EnvVar) {
	// Handle all keys in order, so that any resulting EnvVar definitions are stable.
	template := map[string]any{}
	fixups := []clusterconfiguration.Fixup{}
	envVars := []corev1.EnvVar{}
	for k, v := range helmette.SortedMap(source) {
