...
func TestTemplate(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			objs, decodeErr := kube.DecodeYAML(out, redpandachart.Scheme)
			...
			for _, assertion := range assertions {
				...
				case `ASSERT-NO-GVK`:
					AssertNoGVK(t, redpandachart.Scheme, objs, params)
...
func VersionGoldenTestsCases(t *testing.T) []txtar.File {
	...
	versions := []struct {
		...
		{
			Image:  redpandachart.PartialImage{Tag: ptr.To("v22.1.0")},
			ErrMsg: ptr.To("no longer supported"),
		},
		{
			Image:  redpandachart.PartialImage{Tag: ptr.To("v22.2.0")},
			ErrMsg: ptr.To("does not support TLS on the RPC port. Please upgrade. See technical service bulletin 2023-01."),
		},
		{
			Image:  redpandachart.PartialImage{Tag: ptr.To("v22.3.0")},
			ErrMsg: ptr.To("does not support TLS on the RPC port. Please upgrade. See technical service bulletin 2023-01."),
		},
		{
			Image: redpandachart.PartialImage{Tag: ptr.To("v22.3.14")},
		},
		{
			Image:  redpandachart.PartialImage{Tag: ptr.To("v22.4.0")},
			ErrMsg: ptr.To("does not support TLS on the RPC port. Please upgrade. See technical service bulletin 2023-01."),
		},
		{
			Image:  redpandachart.PartialImage{Tag: ptr.To("v23.1.1")},
			ErrMsg: ptr.To("does not support TLS on the RPC port. Please upgrade. See technical service bulletin 2023-01."),
		},
		{
			Image: redpandachart.PartialImage{Tag: ptr.To("v23.1.2")},
		},
		{
			Image: redpandachart.PartialImage{Tag: ptr.To("v23.1.3")},
		},
		{
			Image: redpandachart.PartialImage{Tag: ptr.To("v23.2.1")},
		},
		{
			Image: redpandachart.PartialImage{Tag: ptr.To("v23.3.0")},
		},
		{
			Image: redpandachart.PartialImage{Tag: ptr.To("v24.1.0")},
		},
		{
			Image: redpandachart.PartialImage{Repository: ptr.To("somecustomrepo"), Tag: ptr.To("v24.1.0")},
		},
		{
			Image: redpandachart.PartialImage{Repository: ptr.To("somecustomrepo"), Tag: ptr.To("v23.2.8")},
		},
	}

	// A collection of features that are protected by the various above version
	// gates.
	permutations := []redpandachart.PartialValues{
		{
			Config: &redpandachart.PartialConfig{
				Tunable: redpandachart.PartialTunableConfig{
		...
		{
			Enterprise: &redpandachart.PartialEnterprise{License: ptr.To("ATOTALLYVALIDLICENSE")},
		},
		{
	...
	for _, version := range versions {
		...
		for i, perm := range permutations {
			values, err := valuesutil.UnmarshalInto[redpandachart.PartialValues](perm)
			require.NoError(t, err)
...
func AssertValidTypes(t *testing.T, objs []kube.Object) {
	...
	for _, obj := range objs {
		...
		gvk, err := apiutil.GVKForObject(obj, redpandachart.Scheme)
		require.NoError(t, err)
