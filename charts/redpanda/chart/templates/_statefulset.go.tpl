...
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (list "sys" "proc" "etc" "usr" "lib" "lib64" "dev" "var" "run")) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
...
{{- $mounts := (list) -}}
{{- range $_, $dir := (get (fromJson (include "redpanda.hostTunerDirs" (dict "a" (list)))) "r") -}}
{{- $mounts = (concat (default (list) $mounts) (list (mustMergeOverwrite (dict "name" "" "mountPath" "") (dict "name" (printf "host-%s" $dir) "mountPath" (printf "/host/%s" $dir) "mountPropagation" "Bidirectional")))) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- $mounts = (concat (default (list) $mounts) (list (mustMergeOverwrite (dict "name" "" "mountPath" "") (dict "name" "host-tuner-state" "mountPath" "/host/tuner_state.yaml" "mountPropagation" "Bidirectional")) (mustMergeOverwrite (dict "name" "" "mountPath" "") (dict "name" "base-config" "mountPath" "/host/redpanda_etc")) (mustMergeOverwrite (dict "name" "" "mountPath" "") (dict "name" "datadir" "mountPath" "/host/var/lib/redpanda/data")))) -}}
{{- $_is_returning = true -}}
...
{{- break -}}
{{- end -}}
{{- $_562_uid_gid := (get (fromJson (include "redpanda.securityContextUidGid" (dict "a" (list $state $pool "set-datadir-ownership")))) "r") -}}
{{- $uid := ((index $_562_uid_gid 0) | int64) -}}
{{- $gid := ((index $_562_uid_gid 1) | int64) -}}
{{- $_is_returning = true -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_588_gid_uid := (get (fromJson (include "redpanda.giduidFromPodTemplate" (dict "a" (list $state.Values.podTemplate "redpanda")))) "r") -}}
{{- $gid := (index $_588_gid_uid 0) -}}
{{- $uid := (index $_588_gid_uid 1) -}}
{{- $_589_sgid_suid := (get (fromJson (include "redpanda.giduidFromPodTemplate" (dict "a" (list $pool.Statefulset.podTemplate "redpanda")))) "r") -}}
{{- $sgid := (index $_589_sgid_suid 0) -}}
{{- $suid := (index $_589_sgid_suid 1) -}}
...
{{- break -}}
{{- end -}}
{{- $_668_uid_gid := (get (fromJson (include "redpanda.securityContextUidGid" (dict "a" (list $state $pool "set-tiered-storage-cache-dir-ownership")))) "r") -}}
{{- $uid := ((index $_668_uid_gid 0) | int64) -}}
{{- $gid := ((index $_668_uid_gid 1) | int64) -}}
