...
{{- end -}}
{{- $volumes = (concat (default (list) $volumes) (list (get (fromJson (include "_redpanda.KubeTokenAPIVolume" (dict "a" (list "kube-api-access")))) "r"))) -}}
{{- if (and $state.Values.tuning.tune_aio_events $state.Values.tuning.apply_host_tuners) -}}
{{- $volumes = (concat (default (list) $volumes) (default (list) (get (fromJson (include "_redpanda.HostTunerVolumes" (dict "a" (list)))) "r"))) -}}
{{- end -}}
{{- $_is_returning = true -}}
...
{{- $_310_uid_gid := (get (fromJson (include "redpanda.securityContextUidGid" (dict "a" (list $state $pool "set-datadir-ownership")))) "r") -}}
{{- $uid := ((index $_310_uid_gid 0) | int64) -}}
{{- $gid := ((index $_310_uid_gid 1) | int64) -}}
...
{{- $_321_uid_gid := (get (fromJson (include "redpanda.securityContextUidGid" (dict "a" (list $state $pool "set-tiered-storage-cache-dir-ownership")))) "r") -}}
{{- $uid := ((index $_321_uid_gid 0) | int64) -}}
{{- $gid := ((index $_321_uid_gid 1) | int64) -}}
{{- $cacheVolumeName := "" -}}
...
{{- end -}}
{{- $_is_returning = true -}}
{{- (dict "r" (get (fromJson (include "_redpanda.StatefulSetInitContainerRenderer.Render" (dict "a" (list (deepCopy $renderer))))) "r")) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_346_gid_uid := (get (fromJson (include "redpanda.giduidFromPodTemplate" (dict "a" (list $state.Values.podTemplate "redpanda")))) "r") -}}
{{- $gid := (index $_346_gid_uid 0) -}}
{{- $uid := (index $_346_gid_uid 1) -}}
{{- $_347_sgid_suid := (get (fromJson (include "redpanda.giduidFromPodTemplate" (dict "a" (list $pool.Statefulset.podTemplate "redpanda")))) "r") -}}
{{- $sgid := (index $_347_sgid_suid 0) -}}
{{- $suid := (index $_347_sgid_suid 1) -}}
...
{{- $_700_uid_gid := (get (fromJson (include "redpanda.securityContextUidGid" (dict "a" (list $state $pool "sidecar")))) "r") -}}
{{- $uid := ((index $_700_uid_gid 0) | int64) -}}
{{- $gid := ((index $_700_uid_gid 1) | int64) -}}
{{- $_is_returning = true -}}
