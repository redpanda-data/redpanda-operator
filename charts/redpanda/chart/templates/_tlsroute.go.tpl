...
{{- $_ := (fail (printf "external listener %s/%s sets gateway: true but external.gateway is not enabled with at least one parentRef; refusing to fall back to a NodePort/LoadBalancer Service. Set external.gateway.enabled: true and external.gateway.parentRefs" $tag $name)) -}}
{{- end -}}
{{- if (eq $host "") -}}
{{- $_ := (fail (printf "external gateway listener %s/%s requires `host` (the bootstrap SNI hostname) when gateway: true" $tag $name)) -}}
{{- end -}}
