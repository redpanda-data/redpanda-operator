...
{{- define "redpanda.templater.Template" -}}
{{- $t := (index .a 0) -}}
{{- $tpl := (index .a 1) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (tpl $tpl $t.Dot)) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
...
{{- $_107_existing_1_ok_2 := (get (fromJson (include "_shims.lookup" (dict "a" (list "v1" "Secret" $r.Release.Namespace $selector.name)))) "r") -}}
{{- $existing_1 := (index $_107_existing_1_ok_2 0) -}}
{{- $ok_2 := (index $_107_existing_1_ok_2 1) -}}
{{- if $ok_2 -}}
...
{{- $_135_data_3_found_4 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $existing_1.data $selector.key (coalesce nil))))) "r") -}}
{{- $data_3 := (index $_135_data_3_found_4 0) -}}
{{- $found_4 := (index $_135_data_3_found_4 1) -}}
{{- if $found_4 -}}
...
{{- $_150_existing_5_ok_6 := (get (fromJson (include "_shims.lookup" (dict "a" (list "apps/v1" "StatefulSet" $r.Release.Namespace (get (fromJson (include "redpanda.Fullname" (dict "a" (list $r)))) "r"))))) "r") -}}
{{- $existing_5 := (index $_150_existing_5_ok_6 0) -}}
{{- $ok_6 := (index $_150_existing_5_ok_6 1) -}}
...
{{- $_198_adminAuthEnabled__ := (get (fromJson (include "_shims.typetest" (dict "a" (list "bool" (index $r.Values.config.cluster "admin_api_require_auth") false)))) "r") -}}
{{- $adminAuthEnabled := (index $_198_adminAuthEnabled__ 0) -}}
{{- $_ := (index $_198_adminAuthEnabled__ 1) -}}
{{- if $adminAuthEnabled -}}
