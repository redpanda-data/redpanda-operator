...
{{- $_ := (set $kafkaSpec "sasl" (mustMergeOverwrite (dict "mechanism" "" "oauth" (dict "tokenSecretRef" (coalesce nil)) "gssapi" (dict "authType" "" "keyTabPath" "" "kerberosConfigPath" "" "serviceName" "" "username" "" "passwordSecretRef" (coalesce nil) "realm" "" "enableFast" false) "awsMskIam" (dict "accessKey" "" "secretKeySecretRef" (coalesce nil) "sessionTokenSecretRef" (coalesce nil) "userAgent" "")) (dict "username" $username "passwordSecretRef" (mustMergeOverwrite (dict) (dict "namespace" $r.Release.Namespace "secretKeyRef" (mustMergeOverwrite (dict "key" "") (mustMergeOverwrite (dict) (dict "name" $passwordRef.name)) (dict "key" $passwordRef.key)))) "mechanism" (toString (get (fromJson (include "redpanda.BootstrapUser.GetMechanism" (dict "a" (list $r.Values.auth.sasl.bootstrapUser)))) "r"))))) -}}
{{- end -}}
...
{{- $_146_adminAuthEnabled__ := (get (fromJson (include "_shims.typetest" (dict "a" (list "bool" (index $r.Values.config.cluster "admin_api_require_auth") false)))) "r") -}}
{{- $adminAuthEnabled := (index $_146_adminAuthEnabled__ 0) -}}
{{- $_ := (index $_146_adminAuthEnabled__ 1) -}}
{{- if $adminAuthEnabled -}}
