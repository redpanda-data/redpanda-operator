...
{{- define "_redpanda.InitContainerRenderer.Render" -}}
{{- $r := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
...
{{- if $r.Tuning.OnHost -}}
{{- $containers = (concat (default (list) $containers) (list (get (fromJson (include "_redpanda.InitContainerRenderer.tuningOnHostContainer" (dict "a" (list $r)))) "r"))) -}}
{{- else -}}
{{- $containers = (concat (default (list) $containers) (list (get (fromJson (include "_redpanda.InitContainerRenderer.tuningContainer" (dict "a" (list $r)))) "r"))) -}}
{{- end -}}
{{- end -}}
{{- if (ne (toJson $r.DataDirOwnership) "null") -}}
{{- $containers = (concat (default (list) $containers) (list (get (fromJson (include "_redpanda.InitContainerRenderer.setDataDirOwnershipContainer" (dict "a" (list $r $r.DataDirOwnership)))) "r"))) -}}
{{- end -}}
{{- if (ne (toJson $r.FSValidator) "null") -}}
{{- $containers = (concat (default (list) $containers) (list (get (fromJson (include "_redpanda.InitContainerRenderer.fsValidatorContainer" (dict "a" (list $r $r.FSValidator)))) "r"))) -}}
{{- end -}}
{{- if (ne (toJson $r.TieredStorageCacheOwnership) "null") -}}
{{- $containers = (concat (default (list) $containers) (list (get (fromJson (include "_redpanda.InitContainerRenderer.setTieredStorageCacheDirOwnershipContainer" (dict "a" (list $r $r.TieredStorageCacheOwnership)))) "r"))) -}}
{{- end -}}
{{- if (ne (toJson $r.Configurator) "null") -}}
{{- $containers = (concat (default (list) $containers) (list (get (fromJson (include "_redpanda.InitContainerRenderer.configuratorContainer" (dict "a" (list $r $r.Configurator)))) "r"))) -}}
{{- end -}}
{{- if (ne (toJson $r.Bootstrap) "null") -}}
{{- $containers = (concat (default (list) $containers) (list (get (fromJson (include "_redpanda.InitContainerRenderer.bootstrapYamlTemplaterContainer" (dict "a" (list $r $r.Bootstrap)))) "r"))) -}}
{{- end -}}
{{- $_is_returning = true -}}
...
{{- define "_redpanda.InitContainerRenderer.mounts" -}}
{{- $r := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $mounts := (coalesce nil) -}}
...
{{- define "_redpanda.InitContainerRenderer.setDataDirOwnershipContainer" -}}
{{- $r := (index .a 0) -}}
{{- $opts := (index .a 1) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (mustMergeOverwrite (dict "name" "" "resources" (dict)) (dict "name" "set-datadir-ownership" "image" $r.InitImage "command" (list `/bin/sh` `-c` (printf `chown %d:%d -R %s` ($opts.UID | int64) ($opts.GID | int64) "/var/lib/redpanda/data")) "securityContext" (mustMergeOverwrite (dict) (dict "runAsUser" (0 | int64) "runAsGroup" (0 | int64))) "volumeMounts" (concat (default (list) (get (fromJson (include "_redpanda.InitContainerRenderer.mounts" (dict "a" (list $r)))) "r")) (list (mustMergeOverwrite (dict "name" "" "mountPath" "") (dict "name" "datadir" "mountPath" "/var/lib/redpanda/data"))))))) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}

{{- define "_redpanda.InitContainerRenderer.fsValidatorContainer" -}}
{{- $r := (index .a 0) -}}
{{- $opts := (index .a 1) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (mustMergeOverwrite (dict "name" "" "resources" (dict)) (dict "name" "fs-validator" "image" $r.Image "command" (list `/bin/sh`) "args" (list `-c` (printf `trap "exit 0" TERM; exec /etc/secrets/fs-validator/scripts/fsValidator.sh %s & wait $!` $opts.ExpectedFS)) "volumeMounts" (concat (default (list) (get (fromJson (include "_redpanda.InitContainerRenderer.mounts" (dict "a" (list $r)))) "r")) (list (mustMergeOverwrite (dict "name" "" "mountPath" "") (dict "name" "fs-validator" "mountPath" `/etc/secrets/fs-validator/scripts/`)) (mustMergeOverwrite (dict "name" "" "mountPath" "") (dict "name" "datadir" "mountPath" "/var/lib/redpanda/data"))))))) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}

{{- define "_redpanda.InitContainerRenderer.configuratorContainer" -}}
{{- $r := (index .a 0) -}}
{{- $opts := (index .a 1) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $volMounts := (concat (default (list) (get (fromJson (include "_redpanda.InitContainerRenderer.mounts" (dict "a" (list $r)))) "r")) (list (mustMergeOverwrite (dict "name" "" "mountPath" "") (dict "name" "config" "mountPath" "/etc/redpanda")) (mustMergeOverwrite (dict "name" "" "mountPath" "") (dict "name" "base-config" "mountPath" "/tmp/base-config")) (mustMergeOverwrite (dict "name" "" "mountPath" "") (dict "name" "configurator" "mountPath" "/etc/secrets/configurator/scripts/")))) -}}
...
{{- define "_redpanda.InitContainerRenderer.setTieredStorageCacheDirOwnershipContainer" -}}
{{- $r := (index .a 0) -}}
{{- $opts := (index .a 1) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $volMounts := (concat (default (list) (get (fromJson (include "_redpanda.InitContainerRenderer.mounts" (dict "a" (list $r)))) "r")) (list (mustMergeOverwrite (dict "name" "" "mountPath" "") (dict "name" "datadir" "mountPath" "/var/lib/redpanda/data")))) -}}
...
{{- define "_redpanda.InitContainerRenderer.bootstrapYamlTemplaterContainer" -}}
{{- $r := (index .a 0) -}}
{{- $opts := (index .a 1) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
