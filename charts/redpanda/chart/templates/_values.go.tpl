...
{{- $result = (concat (default (list) $result) (list (dict "host" (dict "address" (printf "%s-%d.%s" $fullname $i (get (fromJson (include "_shims.ptr_Deref" (dict "a" (list $l.rpc.prefixTemplate $internalDomain)))) "r")) "port" ($l.rpc.port | int))))) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
...
{{- range $_, $item := $items -}}
{{- $_1040___ok_11 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $seen $item.key false)))) "r") -}}
{{- $_ := (index $_1040___ok_11 0) -}}
{{- $ok_11 := (index $_1040___ok_11 1) -}}
{{- if $ok_11 -}}
{{- continue -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1328_cert_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $m $name (dict "enabled" (coalesce nil) "caEnabled" false "applyInternalDNSNames" (coalesce nil) "duration" "" "issuerRef" (coalesce nil) "secretRef" (coalesce nil) "clientSecretRef" (coalesce nil)))))) "r") -}}
{{- $cert := (index $_1328_cert_ok 0) -}}
{{- $ok := (index $_1328_cert_ok 1) -}}
{{- if (not $ok) -}}
...
{{- range $k, $v := $c -}}
{{- if (not (empty $v)) -}}
{{- $_1855___ok_15 := (get (fromJson (include "_shims.asnumeric" (dict "a" (list $v)))) "r") -}}
{{- $_ := ((index $_1855___ok_15 0) | float64) -}}
{{- $ok_15 := (index $_1855___ok_15 1) -}}
{{- if $ok_15 -}}
{{- $_ := (set $result $k $v) -}}
