...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1402_cert_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $m $name (dict "enabled" (coalesce nil) "caEnabled" false "applyInternalDNSNames" (coalesce nil) "duration" "" "issuerRef" (coalesce nil) "secretRef" (coalesce nil) "clientSecretRef" (coalesce nil)))))) "r") -}}
{{- $cert := (index $_1402_cert_ok 0) -}}
{{- $ok := (index $_1402_cert_ok 1) -}}
{{- if (not $ok) -}}
...
{{- continue -}}
{{- end -}}
{{- if (get (fromJson (include "_shims.ptr_Deref" (dict "a" (list $listener.gateway false)))) "r") -}}
{{- continue -}}
{{- end -}}
...
{{- range $k, $v := $c -}}
{{- if (not (empty $v)) -}}
{{- $_1966___ok_15 := (get (fromJson (include "_shims.asnumeric" (dict "a" (list $v)))) "r") -}}
{{- $_ := ((index $_1966___ok_15 0) | float64) -}}
{{- $ok_15 := (index $_1966___ok_15 1) -}}
{{- if $ok_15 -}}
{{- $_ := (set $result $k $v) -}}
...
{{- $result := (dict) -}}
{{- range $k, $v := $c -}}
{{- $_1986_b_16_ok_17 := (get (fromJson (include "_shims.typetest" (dict "a" (list "bool" $v false)))) "r") -}}
{{- $b_16 := (index $_1986_b_16_ok_17 0) -}}
{{- $ok_17 := (index $_1986_b_16_ok_17 1) -}}
{{- if $ok_17 -}}
{{- $_ := (set $result $k $b_16) -}}
{{- continue -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_2031___hasAccessKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_access_key" (coalesce nil))))) "r") -}}
{{- $_ := (index $_2031___hasAccessKey 0) -}}
{{- $hasAccessKey := (index $_2031___hasAccessKey 1) -}}
{{- $_2032___hasSecretKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_secret_key" (coalesce nil))))) "r") -}}
{{- $_ := (index $_2032___hasSecretKey 0) -}}
{{- $hasSecretKey := (index $_2032___hasSecretKey 1) -}}
{{- $_2033___hasSharedKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_azure_shared_key" (coalesce nil))))) "r") -}}
{{- $_ := (index $_2033___hasSharedKey 0) -}}
{{- $hasSharedKey := (index $_2033___hasSharedKey 1) -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_2069___containerExists := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c "cloud_storage_azure_container" (coalesce nil))))) "r") -}}
{{- $_ := (index $_2069___containerExists 0) -}}
{{- $containerExists := (index $_2069___containerExists 1) -}}
{{- $_2070___accountExists := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c "cloud_storage_azure_storage_account" (coalesce nil))))) "r") -}}
{{- $_ := (index $_2070___accountExists 0) -}}
{{- $accountExists := (index $_2070___accountExists 1) -}}
{{- $_is_returning = true -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_2075_value_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c `cloud_storage_cache_size` (coalesce nil))))) "r") -}}
{{- $value := (index $_2075_value_ok 0) -}}
{{- $ok := (index $_2075_value_ok 1) -}}
{{- if (not $ok) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (coalesce nil)) | toJson -}}
{{- break -}}
