...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1323_cert_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $m $name (dict "enabled" (coalesce nil) "caEnabled" false "applyInternalDNSNames" (coalesce nil) "duration" "" "issuerRef" (coalesce nil) "secretRef" (coalesce nil) "clientSecretRef" (coalesce nil) "extraDNSNames" (coalesce nil)))))) "r") -}}
{{- $cert := (index $_1323_cert_ok 0) -}}
{{- $ok := (index $_1323_cert_ok 1) -}}
{{- if (not $ok) -}}
