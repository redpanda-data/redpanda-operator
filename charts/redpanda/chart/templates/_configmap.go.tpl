...
{{- $redpandaYamlFixups = (concat (default (list) $redpandaYamlFixups) (default (list) $fixups)) -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_81_template_fixups := (get (fromJson (include "redpanda.BootstrapContents" (dict "a" (list $state $pool)))) "r") -}}
{{- $template := (index $_81_template_fixups 0) -}}
{{- $fixups := (index $_81_template_fixups 1) -}}
...
{{- $_105_attrs_fixes := (get (fromJson (include "redpanda.TieredStorageConfig.Translate" (dict "a" (list (deepCopy (get (fromJson (include "redpanda.Storage.GetTieredStorageConfig" (dict "a" (list $state.Values.storage)))) "r")) $state.Values.storage.tiered.credentialsSecretRef)))) "r") -}}
{{- $attrs := (index $_105_attrs_fixes 0) -}}
{{- $fixes := (index $_105_attrs_fixes 1) -}}
{{- $bootstrap = (merge (dict) $bootstrap $attrs) -}}
{{- $fixups = (concat (default (list) $fixups) (default (list) $fixes)) -}}
{{- $_115___ok_1 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $state.Values.config.cluster "default_topic_replications" (coalesce nil))))) "r") -}}
{{- $_ := (index $_115___ok_1 0) -}}
{{- $ok_1 := (index $_115___ok_1 1) -}}
{{- if (and (not $ok_1) (ge ($pool.Statefulset.replicas | int) (3 | int))) -}}
{{- $_ := (set $bootstrap "default_topic_replications" (3 | int)) -}}
{{- end -}}
{{- $_120___ok_2 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $state.Values.config.cluster "storage_min_free_bytes" (coalesce nil))))) "r") -}}
{{- $_ := (index $_120___ok_2 0) -}}
{{- $ok_2 := (index $_120___ok_2 1) -}}
{{- if (not $ok_2) -}}
...
{{- break -}}
{{- end -}}
{{- $_131_extra_fixes__ := (get (fromJson (include "redpanda.ClusterConfiguration.Translate" (dict "a" (list (deepCopy $state.Values.config.extraClusterConfiguration))))) "r") -}}
{{- $extra := (index $_131_extra_fixes__ 0) -}}
{{- $fixes := (index $_131_extra_fixes__ 1) -}}
{{- $_ := (index $_131_extra_fixes__ 2) -}}
...
{{- $_220___ok_3 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $kafkaTLS "ca_file" (coalesce nil))))) "r") -}}
{{- $_ := (index $_220___ok_3 0) -}}
{{- $ok_3 := (index $_220___ok_3 1) -}}
{{- if $ok_3 -}}
...
{{- $_226___ok_4 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $adminTLS "ca_file" (coalesce nil))))) "r") -}}
{{- $_ := (index $_226___ok_4 0) -}}
{{- $ok_4 := (index $_226___ok_4 1) -}}
{{- if $ok_4 -}}
...
{{- $_232___ok_5 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $schemaTLS "ca_file" (coalesce nil))))) "r") -}}
{{- $_ := (index $_232___ok_5 0) -}}
{{- $ok_5 := (index $_232___ok_5 1) -}}
{{- if $ok_5 -}}
...
{{- $useLocalhost := false -}}
{{- $_520_val_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $state.Values.config.node $useLocalhostKey (coalesce nil))))) "r") -}}
{{- $val := (index $_520_val_ok 0) -}}
{{- $ok := (index $_520_val_ok 1) -}}
{{- if $ok -}}
{{- if (kindIs "bool" $val) -}}
...
{{- $lockMemory := false -}}
{{- $_706_value_14_ok_15 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $flags "--lock-memory" "")))) "r") -}}
{{- $value_14 := (index $_706_value_14_ok_15 0) -}}
{{- $ok_15 := (index $_706_value_14_ok_15 1) -}}
{{- if $ok_15 -}}
{{- $lockMemory = (ternary (index $enabledOptions $value_14) false (hasKey $enabledOptions $value_14)) -}}
{{- $_ := (unset $flags "--lock-memory") -}}
{{- end -}}
{{- $overprovisioned := false -}}
{{- $_713_value_16_ok_17 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $flags "--overprovisioned" "")))) "r") -}}
