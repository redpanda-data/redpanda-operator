...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_35_bootstrap_fixups := (get (fromJson (include "redpanda.BootstrapFile" (dict "a" (list $state $ss)))) "r") -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_66_template_fixups := (get (fromJson (include "redpanda.BootstrapContents" (dict "a" (list $state $ss)))) "r") -}}
...
{{- $_90_attrs_fixes := (get (fromJson (include "redpanda.TieredStorageConfig.Translate" (dict "a" (list (deepCopy (get (fromJson (include "redpanda.Storage.GetTieredStorageConfig" (dict "a" (list $state.Values.storage)))) "r")) $state.Values.storage.tiered.credentialsSecretRef)))) "r") -}}
...
{{- if (and (not $ok_1) (ge ($ss.Statefulset.replicas | int) (3 | int))) -}}
...
{{- break -}}
{{- end -}}
{{- $_116_extra_fixes__ := (get (fromJson (include "redpanda.ClusterConfiguration.Translate" (dict "a" (list (deepCopy $state.Values.config.extraClusterConfiguration))))) "r") -}}
...
{{- range $_, $set := $state.Pools -}}
{{- $servers = (concat (default (list) $servers) (default (list) (get (fromJson (include "redpanda.Listeners.CreateSeedServers" (dict "a" (list $state.Values.listeners ($set.Statefulset.replicas | int) (get (fromJson (include "redpanda.Fullname" (dict "a" (list $state)))) "r") (get (fromJson (include "redpanda.InternalDomain" (dict "a" (list $state)))) "r"))))) "r"))) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
...
{{- else -}}
{{- $bl = (concat (default (list) $bl) (list (printf "%s%s-%d.%s:%d" (get (fromJson (include "redpanda.Fullname" (dict "a" (list $state)))) "r") (get (fromJson (include "redpanda.NamedStatefulset.Suffix" (dict "a" (list (deepCopy $ss))))) "r") $i (get (fromJson (include "redpanda.InternalDomain" (dict "a" (list $state)))) "r") $port))) -}}
{{- end -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
...
{{- end -}}
{{- $_402_lockMemory_overprovisioned_flags := (get (fromJson (include "redpanda.RedpandaAdditionalStartFlags" (dict "a" (list $state.Values)))) "r") -}}
