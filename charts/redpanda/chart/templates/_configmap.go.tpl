...
{{- $_48_f__ := (get (fromJson (include "redpanda.SASLClientFixups" (dict "a" (list "schema_registry_client" $state.Values.config.schema_registry_client.saslSecretRef.name)))) "r") -}}
{{- $f := (index $_48_f__ 0) -}}
{{- $_ := (index $_48_f__ 1) -}}
{{- $_ := (set $data "redpanda.yaml.fixups" (toJson $f)) -}}
{{- end -}}
{{- $_is_returning = true -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_78_template_fixups := (get (fromJson (include "redpanda.BootstrapContents" (dict "a" (list $state $pool)))) "r") -}}
{{- $template := (index $_78_template_fixups 0) -}}
{{- $fixups := (index $_78_template_fixups 1) -}}
...
{{- $_102_attrs_fixes := (get (fromJson (include "redpanda.TieredStorageConfig.Translate" (dict "a" (list (deepCopy (get (fromJson (include "redpanda.Storage.GetTieredStorageConfig" (dict "a" (list $state.Values.storage)))) "r")) $state.Values.storage.tiered.credentialsSecretRef)))) "r") -}}
{{- $attrs := (index $_102_attrs_fixes 0) -}}
{{- $fixes := (index $_102_attrs_fixes 1) -}}
{{- $bootstrap = (merge (dict) $bootstrap $attrs) -}}
{{- $fixups = (concat (default (list) $fixups) (default (list) $fixes)) -}}
{{- $_112___ok_1 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $state.Values.config.cluster "default_topic_replications" (coalesce nil))))) "r") -}}
{{- $_ := (index $_112___ok_1 0) -}}
{{- $ok_1 := (index $_112___ok_1 1) -}}
{{- if (and (not $ok_1) (ge ($pool.Statefulset.replicas | int) (3 | int))) -}}
{{- $_ := (set $bootstrap "default_topic_replications" (3 | int)) -}}
{{- end -}}
{{- $_117___ok_2 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $state.Values.config.cluster "storage_min_free_bytes" (coalesce nil))))) "r") -}}
{{- $_ := (index $_117___ok_2 0) -}}
{{- $ok_2 := (index $_117___ok_2 1) -}}
{{- if (not $ok_2) -}}
...
{{- $_217___ok_3 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $kafkaTLS "ca_file" (coalesce nil))))) "r") -}}
{{- $_ := (index $_217___ok_3 0) -}}
{{- $ok_3 := (index $_217___ok_3 1) -}}
{{- if $ok_3 -}}
...
{{- $_223___ok_4 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $adminTLS "ca_file" (coalesce nil))))) "r") -}}
{{- $_ := (index $_223___ok_4 0) -}}
{{- $ok_4 := (index $_223___ok_4 1) -}}
{{- if $ok_4 -}}
...
{{- $_229___ok_5 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $schemaTLS "ca_file" (coalesce nil))))) "r") -}}
{{- $_ := (index $_229___ok_5 0) -}}
{{- $ok_5 := (index $_229___ok_5 1) -}}
{{- if $ok_5 -}}
...
{{- $lockMemory := false -}}
{{- $_703_value_14_ok_15 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $flags "--lock-memory" "")))) "r") -}}
{{- $value_14 := (index $_703_value_14_ok_15 0) -}}
{{- $ok_15 := (index $_703_value_14_ok_15 1) -}}
{{- if $ok_15 -}}
{{- $lockMemory = (ternary (index $enabledOptions $value_14) false (hasKey $enabledOptions $value_14)) -}}
{{- $_ := (unset $flags "--lock-memory") -}}
{{- end -}}
{{- $overprovisioned := false -}}
{{- $_710_value_16_ok_17 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $flags "--overprovisioned" "")))) "r") -}}
{{- $value_16 := (index $_710_value_16_ok_17 0) -}}
{{- $ok_17 := (index $_710_value_16_ok_17 1) -}}
{{- if $ok_17 -}}
