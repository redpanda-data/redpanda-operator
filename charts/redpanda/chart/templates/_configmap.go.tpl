...
{{- $cm := (mustMergeOverwrite (dict "metadata" (dict)) (mustMergeOverwrite (dict) (dict "kind" "ConfigMap" "apiVersion" "v1")) (dict "metadata" (mustMergeOverwrite (dict) (dict "name" (printf "%s%s" (get (fromJson (include "redpanda.Fullname" (dict "a" (list $state)))) "r") (get (fromJson (include "redpanda.Pool.Suffix" (dict "a" (list (deepCopy $pool))))) "r")) "namespace" $state.Release.Namespace "labels" (get (fromJson (include "redpanda.FullLabels" (dict "a" (list $state)))) "r"))) "data" (dict ".bootstrap.json.in" $bootstrap "bootstrap.yaml.fixups" $fixups "redpanda.yaml" (get (fromJson (include "redpanda.RedpandaConfigFile" (dict "a" (list $state true $pool)))) "r")))) -}}
{{- $_ := (get (fromJson (include "redpanda.annotate" (dict "a" (list $state $cm.metadata)))) "r") -}}
{{- $_is_returning = true -}}
{{- (dict "r" $cm) | toJson -}}
{{- break -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_71_template_fixups := (get (fromJson (include "redpanda.BootstrapContents" (dict "a" (list $state $pool)))) "r") -}}
{{- $template := (index $_71_template_fixups 0) -}}
{{- $fixups := (index $_71_template_fixups 1) -}}
...
{{- $_95_attrs_fixes := (get (fromJson (include "redpanda.TieredStorageConfig.Translate" (dict "a" (list (deepCopy (get (fromJson (include "redpanda.Storage.GetTieredStorageConfig" (dict "a" (list $state.Values.storage)))) "r")) $state.Values.storage.tiered.credentialsSecretRef)))) "r") -}}
{{- $attrs := (index $_95_attrs_fixes 0) -}}
{{- $fixes := (index $_95_attrs_fixes 1) -}}
{{- range $k, $v := $attrs -}}
{{- $_103___ok_1 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $bootstrap $k (coalesce nil))))) "r") -}}
{{- $_ := (index $_103___ok_1 0) -}}
{{- $ok_1 := (index $_103___ok_1 1) -}}
{{- if (not $ok_1) -}}
{{- $_ := (set $bootstrap $k $v) -}}
{{- end -}}
{{- end -}}
...
{{- $_116___ok_2 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $state.Values.config.cluster "default_topic_replications" (coalesce nil))))) "r") -}}
{{- $_ := (index $_116___ok_2 0) -}}
{{- $ok_2 := (index $_116___ok_2 1) -}}
{{- if (and (not $ok_2) (ge ($pool.Statefulset.replicas | int) (3 | int))) -}}
{{- $_ := (set $bootstrap "default_topic_replications" (3 | int)) -}}
{{- end -}}
{{- $_121___ok_3 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $state.Values.config.cluster "storage_min_free_bytes" (coalesce nil))))) "r") -}}
{{- $_ := (index $_121___ok_3 0) -}}
{{- $ok_3 := (index $_121___ok_3 1) -}}
{{- if (not $ok_3) -}}
...
{{- break -}}
{{- end -}}
{{- $_132_extra_fixes__ := (get (fromJson (include "redpanda.ClusterConfiguration.Translate" (dict "a" (list (deepCopy $state.Values.config.extraClusterConfiguration))))) "r") -}}
{{- $extra := (index $_132_extra_fixes__ 0) -}}
{{- $fixes := (index $_132_extra_fixes__ 1) -}}
{{- $_ := (index $_132_extra_fixes__ 2) -}}
...
{{- break -}}
{{- end -}}
{{- $cm := (mustMergeOverwrite (dict "metadata" (dict)) (mustMergeOverwrite (dict) (dict "kind" "ConfigMap" "apiVersion" "v1")) (dict "metadata" (mustMergeOverwrite (dict) (dict "name" (printf "%s-rpk" (get (fromJson (include "redpanda.Fullname" (dict "a" (list $state)))) "r")) "namespace" $state.Release.Namespace "labels" (get (fromJson (include "redpanda.FullLabels" (dict "a" (list $state)))) "r"))) "data" (dict "profile" (toYaml (get (fromJson (include "redpanda.rpkProfile" (dict "a" (list $state)))) "r"))))) -}}
{{- $_ := (get (fromJson (include "redpanda.annotate" (dict "a" (list $state $cm.metadata)))) "r") -}}
{{- $_is_returning = true -}}
{{- (dict "r" $cm) | toJson -}}
{{- break -}}
...
{{- $_225___ok_4 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $kafkaTLS "ca_file" (coalesce nil))))) "r") -}}
...
{{- $_231___ok_5 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $adminTLS "ca_file" (coalesce nil))))) "r") -}}
...
{{- $_237___ok_6 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $schemaTLS "ca_file" (coalesce nil))))) "r") -}}
{{- $_ := (index $_237___ok_6 0) -}}
{{- $ok_6 := (index $_237___ok_6 1) -}}
{{- if $ok_6 -}}
...
{{- end -}}
{{- $_424_lockMemory_overprovisioned_flags := (get (fromJson (include "redpanda.RedpandaAdditionalStartFlags" (dict "a" (list $state.Values $pool)))) "r") -}}
{{- $lockMemory := (index $_424_lockMemory_overprovisioned_flags 0) -}}
{{- $overprovisioned := (index $_424_lockMemory_overprovisioned_flags 1) -}}
{{- $flags := (index $_424_lockMemory_overprovisioned_flags 2) -}}
...
{{- $useLocalhost := false -}}
{{- $_534_val_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $state.Values.config.node $useLocalhostKey (coalesce nil))))) "r") -}}
{{- $val := (index $_534_val_ok 0) -}}
{{- $ok := (index $_534_val_ok 1) -}}
{{- if $ok -}}
{{- if (kindIs "bool" $val) -}}
...
{{- $lockMemory := false -}}
{{- $_720_value_15_ok_16 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $flags "--lock-memory" "")))) "r") -}}
{{- $value_15 := (index $_720_value_15_ok_16 0) -}}
{{- $ok_16 := (index $_720_value_15_ok_16 1) -}}
{{- if $ok_16 -}}
{{- $lockMemory = (ternary (index $enabledOptions $value_15) false (hasKey $enabledOptions $value_15)) -}}
{{- $_ := (unset $flags "--lock-memory") -}}
{{- end -}}
{{- $overprovisioned := false -}}
{{- $_727_value_17_ok_18 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $flags "--overprovisioned" "")))) "r") -}}
{{- $value_17 := (index $_727_value_17_ok_18 0) -}}
{{- $ok_18 := (index $_727_value_17_ok_18 1) -}}
{{- if $ok_18 -}}
