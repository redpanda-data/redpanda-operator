...
{{- break -}}
{{- end -}}
{{- $_123_extra_fixes__ := (get (fromJson (include "redpanda.ClusterConfiguration.Translate" (dict "a" (list (deepCopy $state.Values.config.extraClusterConfiguration))))) "r") -}}
{{- $extra := (index $_123_extra_fixes__ 0) -}}
{{- $fixes := (index $_123_extra_fixes__ 1) -}}
{{- $_ := (index $_123_extra_fixes__ 2) -}}
...
{{- $_212___ok_3 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $kafkaTLS "ca_file" (coalesce nil))))) "r") -}}
{{- $_ := (index $_212___ok_3 0) -}}
{{- $ok_3 := (index $_212___ok_3 1) -}}
{{- if $ok_3 -}}
...
{{- $_218___ok_4 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $adminTLS "ca_file" (coalesce nil))))) "r") -}}
...
{{- $_224___ok_5 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $schemaTLS "ca_file" (coalesce nil))))) "r") -}}
...
{{- end -}}
{{- $_411_lockMemory_overprovisioned_flags := (get (fromJson (include "redpanda.RedpandaAdditionalStartFlags" (dict "a" (list $state.Values $pool)))) "r") -}}
{{- $lockMemory := (index $_411_lockMemory_overprovisioned_flags 0) -}}
{{- $overprovisioned := (index $_411_lockMemory_overprovisioned_flags 1) -}}
{{- $flags := (index $_411_lockMemory_overprovisioned_flags 2) -}}
...
{{- $useLocalhost := false -}}
{{- $_512_val_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $state.Values.config.node $useLocalhostKey (coalesce nil))))) "r") -}}
{{- $val := (index $_512_val_ok 0) -}}
{{- $ok := (index $_512_val_ok 1) -}}
{{- if $ok -}}
{{- if (kindIs "bool" $val) -}}
...
{{- $lockMemory := false -}}
{{- $_698_value_14_ok_15 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $flags "--lock-memory" "")))) "r") -}}
{{- $value_14 := (index $_698_value_14_ok_15 0) -}}
{{- $ok_15 := (index $_698_value_14_ok_15 1) -}}
{{- if $ok_15 -}}
{{- $lockMemory = (ternary (index $enabledOptions $value_14) false (hasKey $enabledOptions $value_14)) -}}
{{- $_ := (unset $flags "--lock-memory") -}}
{{- end -}}
{{- $overprovisioned := false -}}
{{- $_705_value_16_ok_17 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $flags "--overprovisioned" "")))) "r") -}}
{{- $value_16 := (index $_705_value_16_ok_17 0) -}}
{{- $ok_17 := (index $_705_value_16_ok_17 1) -}}
{{- if $ok_17 -}}
