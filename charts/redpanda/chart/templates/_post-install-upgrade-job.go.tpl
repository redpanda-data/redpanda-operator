...
{{- $_ := (get (fromJson (include "redpanda.annotate" (dict "a" (list $state $job.metadata)))) "r") -}}
{{- $_ := (get (fromJson (include "redpanda.annotate" (dict "a" (list $state $job.spec.template.metadata)))) "r") -}}
{{- $_is_returning = true -}}
{{- (dict "r" $job) | toJson -}}
{{- break -}}
{{- end -}}
