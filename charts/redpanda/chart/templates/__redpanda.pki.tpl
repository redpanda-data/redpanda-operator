...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_55_cert_1_ok_2 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $p.Certificates $name (dict "Server" (dict "Name" "" "Secret" (dict) "Request" (coalesce nil) "CA" (coalesce nil)) "Client" (coalesce nil)))))) "r") -}}
{{- $cert_1 := (index $_55_cert_1_ok_2 0) -}}
{{- $ok_2 := (index $_55_cert_1_ok_2 1) -}}
{{- if (and $ok_2 (ne (toJson $cert_1.Client) "null")) -}}
{{- $_is_returning = true -}}
{{- (dict "r" $cert_1.Client) | toJson -}}
{{- break -}}
{{- end -}}
{{- $_is_returning = true -}}
{{- (dict "r" (mustMergeOverwrite (dict "Name" "" "Secret" (dict) "Request" (coalesce nil) "CA" (coalesce nil)) (dict "Name" (get (fromJson (include "_redpanda.ClientKeypairName" (dict "a" (list $name)))) "r")))) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
...
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (mustMergeOverwrite (dict "metadata" (dict) "spec" (dict "secretName" "" "issuerRef" (dict "name" "")) "status" (dict)) (mustMergeOverwrite (dict) (dict "apiVersion" "cert-manager.io/v1" "kind" "Certificate")) (dict "metadata" (mustMergeOverwrite (dict) (dict "name" $k.Request.objectName "namespace" $p.Namespace "labels" $p.Labels)) "spec" (mustMergeOverwrite (dict "secretName" "" "issuerRef" (dict "name" "")) (dict "dnsNames" $k.Request.dnsNames "duration" $k.Request.duration "isCA" false "issuerRef" $k.Request.issuerRef "secretName" $k.Secret.name "privateKey" (mustMergeOverwrite (dict) (dict "algorithm" "ECDSA" "size" (256 | int)))))))) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
...
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (mustMergeOverwrite (dict "metadata" (dict) "spec" (dict "secretName" "" "issuerRef" (dict "name" "")) "status" (dict)) (mustMergeOverwrite (dict) (dict "apiVersion" "cert-manager.io/v1" "kind" "Certificate")) (dict "metadata" (mustMergeOverwrite (dict) (dict "name" $k.Request.objectName "namespace" $p.Namespace "labels" $p.Labels)) "spec" (mustMergeOverwrite (dict "secretName" "" "issuerRef" (dict "name" "")) (dict "commonName" $k.Request.commonName "duration" $k.Request.duration "isCA" false "secretName" $k.Secret.name "privateKey" (mustMergeOverwrite (dict) (dict "algorithm" "ECDSA" "size" (256 | int))) "issuerRef" $k.Request.issuerRef))))) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
...
{{- define "_redpanda.ClientKeypairName" -}}
{{- $certName := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (printf "%s-client" $certName)) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
