...
{{- $_ := (get (fromJson (include "redpandav25.RenderState.FetchBootstrapUser" (dict "a" (list $state)))) "r") -}}
{{- $_ := (get (fromJson (include "redpandav25.RenderState.FetchStatefulSetPodSelector" (dict "a" (list $state)))) "r") -}}
{{- $manifests := (get (fromJson (include "redpandav25.renderResources" (dict "a" (list $state)))) "r") -}}
{{- range $_, $obj := (get (fromJson (include "redpandav25.StatefulSets" (dict "a" (list $state)))) "r") -}}
...
{{- define "redpandav25.renderResources" -}}
{{- $state := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_ := (get (fromJson (include "redpandav25.checkVersion" (dict "a" (list $state)))) "r") -}}
{{- $manifests := (list (get (fromJson (include "redpandav25.NodePortService" (dict "a" (list $state)))) "r") (get (fromJson (include "redpandav25.PodDisruptionBudget" (dict "a" (list $state)))) "r") (get (fromJson (include "redpandav25.ServiceAccount" (dict "a" (list $state)))) "r") (get (fromJson (include "redpandav25.ServiceInternal" (dict "a" (list $state)))) "r") (get (fromJson (include "redpandav25.ServiceMonitor" (dict "a" (list $state)))) "r") (get (fromJson (include "redpandav25.PostInstallUpgradeJob" (dict "a" (list $state)))) "r")) -}}
{{- range $_, $obj := (get (fromJson (include "redpandav25.ConfigMaps" (dict "a" (list $state)))) "r") -}}
{{- $manifests = (concat (default (list) $manifests) (list $obj)) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- range $_, $obj := (get (fromJson (include "redpandav25.CertIssuers" (dict "a" (list $state)))) "r") -}}
{{- $manifests = (concat (default (list) $manifests) (list $obj)) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- range $_, $obj := (get (fromJson (include "redpandav25.RootCAs" (dict "a" (list $state)))) "r") -}}
{{- $manifests = (concat (default (list) $manifests) (list $obj)) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- range $_, $obj := (get (fromJson (include "redpandav25.ClientCerts" (dict "a" (list $state)))) "r") -}}
{{- $manifests = (concat (default (list) $manifests) (list $obj)) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- range $_, $obj := (get (fromJson (include "redpandav25.Roles" (dict "a" (list $state)))) "r") -}}
{{- $manifests = (concat (default (list) $manifests) (list $obj)) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- range $_, $obj := (get (fromJson (include "redpandav25.ClusterRoles" (dict "a" (list $state)))) "r") -}}
{{- $manifests = (concat (default (list) $manifests) (list $obj)) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- range $_, $obj := (get (fromJson (include "redpandav25.RoleBindings" (dict "a" (list $state)))) "r") -}}
{{- $manifests = (concat (default (list) $manifests) (list $obj)) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- range $_, $obj := (get (fromJson (include "redpandav25.ClusterRoleBindings" (dict "a" (list $state)))) "r") -}}
{{- $manifests = (concat (default (list) $manifests) (list $obj)) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- range $_, $obj := (get (fromJson (include "redpandav25.LoadBalancerServices" (dict "a" (list $state)))) "r") -}}
{{- $manifests = (concat (default (list) $manifests) (list $obj)) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- range $_, $obj := (get (fromJson (include "redpandav25.Secrets" (dict "a" (list $state)))) "r") -}}
{{- $manifests = (concat (default (list) $manifests) (list $obj)) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- $manifests = (concat (default (list) $manifests) (default (list) (get (fromJson (include "redpandav25.consoleChartIntegration" (dict "a" (list $state)))) "r"))) -}}
{{- $_is_returning = true -}}
{{- (dict "r" $manifests) | toJson -}}
{{- break -}}
{{- end -}}
...
{{- define "redpandav25.checkVersion" -}}
{{- $state := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- if (and (not (get (fromJson (include "redpandav25.RedpandaAtLeast_22_2_0" (dict "a" (list $state)))) "r")) (not $state.Values.force)) -}}
{{- $sv := (get (fromJson (include "redpandav25.semver" (dict "a" (list $state)))) "r") -}}
