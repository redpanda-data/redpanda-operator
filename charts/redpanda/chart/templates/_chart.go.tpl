...
{{- $pki := (get (fromJson (include "redpanda.resolvePKI" (dict "a" (list $state)))) "r") -}}
{{- $listeners := (get (fromJson (include "redpanda.resolveListeners" (dict "a" (list $state $pki)))) "r") -}}
{{- $manifests := (get (fromJson (include "redpanda.renderResources" (dict "a" (list $state $pki $listeners)))) "r") -}}
{{- range $_, $obj := (get (fromJson (include "redpanda.StatefulSets" (dict "a" (list $state $pki $listeners)))) "r") -}}
