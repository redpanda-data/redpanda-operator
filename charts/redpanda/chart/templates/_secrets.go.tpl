...
{{- $secrets = (concat (default (list) $secrets) (list (get (fromJson (include "redpanda.SecretSTSLifecycle" (dict "a" (list $state $listeners)))) "r"))) -}}
...
{{- break -}}
{{- end -}}
{{- $adminCurlFlags := (get (fromJson (include "_redpanda.API.CurlFlags" (dict "a" (list (get (fromJson (include "_redpanda.Listeners.Admin" (dict "a" (list $listeners)))) "r"))))) "r") -}}
...
{{- end -}}
{{- $snippet = (concat (default (list) $snippet) (list `` (printf `PREFIX_TEMPLATE=%s` (quote $prefixTemplate)) (printf `ADVERTISED_%s_ADDRESSES+=(%s)` $arrayName (quote $address)))) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
{{- $snippet = (concat (default (list) $snippet) (list `` (printf `rpk redpanda config --config "$CONFIG" set %s.%s[%d] "${ADVERTISED_%s_ADDRESSES[$POD_ORDINAL]}"` (get (fromJson (include "_redpanda.APIKind.AdvertisedConfigSection" (dict "a" (list (deepCopy $api.Kind))))) "r") (get (fromJson (include "_redpanda.APIKind.AdvertisedConfigKey" (dict "a" (list (deepCopy $api.Kind))))) "r") $externalCounter $arrayName))) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- if (and (get (fromJson (include "redpanda.ExternalConfig.IsGatewayEnabled" (dict "a" (list $state.Values.external)))) "r") (ne (toJson $gateway) "null")) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (get (fromJson (include "redpanda.advertisedHostJSONGateway" (dict "a" (list $state $name $globalOrdinal $gateway)))) "r")) | toJson -}}
{{- break -}}
{{- end -}}
...
{{- if (lt $globalOrdinal ((get (fromJson (include "_shims.len" (dict "a" (list $gateway.BrokerHosts)))) "r") | int)) -}}
{{- $address = (index $gateway.BrokerHosts $globalOrdinal) -}}
{{- end -}}
{{- $_is_returning = true -}}
