...
func newClient(t *testing.T, ctl *kube.Ctl, release *helm.Release, values any) *Client {
	dot, err := redpandachart.Chart.Dot(
		ctl.RestConfig(),
...
func (c *Client) GetClusterHealth(ctx context.Context) (rpadmin.ClusterHealthOverview, error) {
	...
	adminClient, err := redpandaclient.AdminClient(c.state, dialer.DialContext)
	if err != nil {
...
func (c *Client) QuerySupportedFormats(ctx context.Context) ([]string, error) {
	...
	srClient, err := redpandaclient.SchemaRegistryClient(c.state, dialer.DialContext)
	if err != nil {
		return nil, err
	}
...
func (c *Client) createClient(ctx context.Context, ports []portforward.ForwardedPort, cfg redpandachart.ListenerConfig[string]) (*portForwardClient, error) {
	schema := "http"
