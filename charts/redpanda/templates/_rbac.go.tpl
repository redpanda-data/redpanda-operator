...
{{- $mapping := (dict "files/sidecar.Role.yaml" $values.rbac.enabled "files/pvcunbinder.Role.yaml" (and $values.rbac.enabled $values.statefulset.sideCars.pvcUnbinder.enabled) "files/decommission.Role.yaml" (and $values.rbac.enabled $values.statefulset.sideCars.brokerDecommissioner.enabled) "files/rpk-debug-bundle.Role.yaml" (and $values.rbac.enabled $values.rbac.rpkDebugBundle)) -}}
{{- $roles := (coalesce nil) -}}
