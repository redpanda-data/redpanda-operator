...
{{- $_84_existing_1_ok_2 := (get (fromJson (include "_shims.lookup" (dict "a" (list "apps/v1" "StatefulSet" $dot.Release.Namespace (get (fromJson (include "redpanda.Fullname" (dict "a" (list $dot) ))) "r")) ))) "r") -}}
{{- $existing_1 := (index $_84_existing_1_ok_2 0) -}}
{{- $ok_2 := (index $_84_existing_1_ok_2 1) -}}
...
{{- $_is_returning := false -}}
{{- if $dot.Release.IsUpgrade -}}
{{- $_115_existing_3_ok_4 := (get (fromJson (include "_shims.lookup" (dict "a" (list "apps/v1" "StatefulSet" $dot.Release.Namespace (get (fromJson (include "redpanda.Fullname" (dict "a" (list $dot) ))) "r")) ))) "r") -}}
{{- $existing_3 := (index $_115_existing_3_ok_4 0) -}}
{{- $ok_4 := (index $_115_existing_3_ok_4 1) -}}
...
{{- break -}}
{{- end -}}
{{- $_451_uid_gid := (get (fromJson (include "redpanda.securityContextUidGid" (dict "a" (list $dot "set-datadir-ownership") ))) "r") -}}
{{- $uid := ((index $_451_uid_gid 0) | int64) -}}
{{- $gid := ((index $_451_uid_gid 1) | int64) -}}
{{- $_is_returning = true -}}
...
{{- break -}}
{{- end -}}
{{- $_532_uid_gid := (get (fromJson (include "redpanda.securityContextUidGid" (dict "a" (list $dot "set-tiered-storage-cache-dir-ownership") ))) "r") -}}
{{- $uid := ((index $_532_uid_gid 0) | int64) -}}
{{- $gid := ((index $_532_uid_gid 1) | int64) -}}
