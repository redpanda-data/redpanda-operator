...
{{- $_is_returning := false -}}
{{- $values := $dot.Values.AsMap -}}
{{- $_462_gid_uid := (get (fromJson (include "redpanda.giduidFromPodTemplate" (dict "a" (list $values.podTemplate "redpanda") ))) "r") -}}
{{- $gid := (index $_462_gid_uid 0) -}}
{{- $uid := (index $_462_gid_uid 1) -}}
{{- $_463_sgid_suid := (get (fromJson (include "redpanda.giduidFromPodTemplate" (dict "a" (list $values.statefulset.podTemplate "redpanda") ))) "r") -}}
{{- $sgid := (index $_463_sgid_suid 0) -}}
{{- $suid := (index $_463_sgid_suid 1) -}}
...
{{- break -}}
{{- end -}}
{{- $_547_uid_gid := (get (fromJson (include "redpanda.securityContextUidGid" (dict "a" (list $dot "set-tiered-storage-cache-dir-ownership") ))) "r") -}}
{{- $uid := ((index $_547_uid_gid 0) | int64) -}}
{{- $gid := ((index $_547_uid_gid 1) | int64) -}}
...
{{- $_is_returning := false -}}
{{- $values := $dot.Values.AsMap -}}
{{- $args := (list `sidecar` `--redpanda-yaml` `/etc/redpanda/redpanda.yaml` `--redpanda-cluster-namespace` $dot.Release.Namespace `--redpanda-cluster-name` (get (fromJson (include "redpanda.Fullname" (dict "a" (list $dot) ))) "r") `--run-broker-probe` `--broker-probe-broker-url` (get (fromJson (include "redpanda.adminURLsCLI" (dict "a" (list $dot) ))) "r")) -}}
