...
{{- define "redpanda.RedpandaResources.GetRedpandaStartFlags" -}}
{{- $rr := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $flags := (dict ) -}}
{{- $coresInMillies_2 := ((get (fromJson (include "_shims.resource_MilliValue" (dict "a" (list $rr.cpu.cores) ))) "r") | int64) -}}
{{- if (lt $coresInMillies_2 (1000 | int64)) -}}
{{- $_ := (set $flags "smp" (printf "%d" (1 | int))) -}}
{{- else -}}
{{- $_ := (set $flags "smp" (printf "%d" ((get (fromJson (include "_shims.resource_Value" (dict "a" (list $rr.cpu.cores) ))) "r") | int64))) -}}
{{- end -}}
{{- $_ := (set $flags "memory" (printf "%dM" ((get (fromJson (include "redpanda.RedpandaResources.memory" (dict "a" (list $rr) ))) "r") | int64))) -}}
{{- $_ := (set $flags "reserve-memory" (printf "%dM" ((get (fromJson (include "redpanda.RedpandaResources.reserveMemory" (dict "a" (list $rr) ))) "r") | int64))) -}}
{{- $_is_returning = true -}}
{{- (dict "r" $flags) | toJson -}}
{{- break -}}
{{- end -}}
...
{{- $rpMem_3 := $rr.memory.redpanda -}}
{{- if (and (ne (toJson $rpMem_3) "null") (ne (toJson $rpMem_3.memory) "null")) -}}
{{- $memory = ((div ((get (fromJson (include "_shims.resource_Value" (dict "a" (list $rpMem_3.memory) ))) "r") | int64) ((mul (1024 | int) (1024 | int)))) | int64) -}}
{{- else -}}
...
{{- end -}}
{{- if (gt ((add $memory ((get (fromJson (include "redpanda.RedpandaResources.reserveMemory" (dict "a" (list $rr) ))) "r") | int64)) | int64) $containerMemory) -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $rpMem_4 := $rr.memory.redpanda -}}
{{- if (and (ne (toJson $rpMem_4) "null") (ne (toJson $rpMem_4.reserveMemory) "null")) -}}
{{- $_is_returning = true -}}
{{- (dict "r" ((div ((get (fromJson (include "_shims.resource_Value" (dict "a" (list $rpMem_4.reserveMemory) ))) "r") | int64) ((mul (1024 | int) (1024 | int)))) | int64)) | toJson -}}
{{- break -}}
{{- end -}}
{{- $_is_returning = true -}}
...
{{- $_514_b_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $conf "cloud_storage_enabled" (coalesce nil)) ))) "r") -}}
{{- $b := (index $_514_b_ok 0) -}}
{{- $ok := (index $_514_b_ok 1) -}}
{{- $_is_returning = true -}}
...
{{- $_543_dir_5_ok_6 := (get (fromJson (include "_shims.typetest" (dict "a" (list "string" (index $values.config.node "cloud_storage_cache_directory") "") ))) "r") -}}
{{- $dir_5 := (index $_543_dir_5_ok_6 0) -}}
{{- $ok_6 := (index $_543_dir_5_ok_6 1) -}}
{{- if $ok_6 -}}
{{- $_is_returning = true -}}
{{- (dict "r" $dir_5) | toJson -}}
{{- break -}}
...
{{- $_552_dir_7_ok_8 := (get (fromJson (include "_shims.typetest" (dict "a" (list "string" (index $tieredConfig "cloud_storage_cache_directory") "") ))) "r") -}}
{{- $dir_7 := (index $_552_dir_7_ok_8 0) -}}
{{- $ok_8 := (index $_552_dir_7_ok_8 1) -}}
{{- if $ok_8 -}}
{{- $_is_returning = true -}}
{{- (dict "r" $dir_7) | toJson -}}
{{- break -}}
...
{{- $s := (toJson $t) -}}
{{- $tune := (fromJson $s) -}}
{{- $_778_m_ok := (get (fromJson (include "_shims.typetest" (dict "a" (list (printf "map[%s]%s" "string" "interface {}") $tune (coalesce nil)) ))) "r") -}}
{{- $m := (index $_778_m_ok 0) -}}
{{- $ok := (index $_778_m_ok 1) -}}
{{- if (not $ok) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (dict )) | toJson -}}
{{- break -}}
...
{{- range $_, $item := $items -}}
{{- $_895___ok_9 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $seen $item.key false) ))) "r") -}}
{{- $_ := (index $_895___ok_9 0) -}}
{{- $ok_9 := (index $_895___ok_9 1) -}}
{{- if $ok_9 -}}
{{- continue -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_986_cert_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $m $name (dict "enabled" (coalesce nil) "caEnabled" false "applyInternalDNSNames" (coalesce nil) "duration" "" "issuerRef" (coalesce nil) "secretRef" (coalesce nil) "clientSecretRef" (coalesce nil) )) ))) "r") -}}
{{- $cert := (index $_986_cert_ok 0) -}}
{{- $ok := (index $_986_cert_ok 1) -}}
{{- if (not $ok) -}}
...
{{- range $k, $v := $c -}}
{{- if (not (empty $v)) -}}
{{- $_1820___ok_16 := (get (fromJson (include "_shims.asnumeric" (dict "a" (list $v) ))) "r") -}}
{{- $_ := ((index $_1820___ok_16 0) | float64) -}}
{{- $ok_16 := (index $_1820___ok_16 1) -}}
{{- if $ok_16 -}}
{{- $_ := (set $result $k $v) -}}
...
{{- $result := (dict ) -}}
{{- range $k, $v := $c -}}
{{- $_1840_b_17_ok_18 := (get (fromJson (include "_shims.typetest" (dict "a" (list "bool" $v false) ))) "r") -}}
{{- $b_17 := (index $_1840_b_17_ok_18 0) -}}
{{- $ok_18 := (index $_1840_b_17_ok_18 1) -}}
{{- if $ok_18 -}}
{{- $_ := (set $result $k $b_17) -}}
{{- continue -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1885___hasAccessKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_access_key" (coalesce nil)) ))) "r") -}}
{{- $_ := (index $_1885___hasAccessKey 0) -}}
{{- $hasAccessKey := (index $_1885___hasAccessKey 1) -}}
{{- $_1886___hasSecretKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_secret_key" (coalesce nil)) ))) "r") -}}
{{- $_ := (index $_1886___hasSecretKey 0) -}}
{{- $hasSecretKey := (index $_1886___hasSecretKey 1) -}}
{{- $_1887___hasSharedKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_azure_shared_key" (coalesce nil)) ))) "r") -}}
{{- $_ := (index $_1887___hasSharedKey 0) -}}
{{- $hasSharedKey := (index $_1887___hasSharedKey 1) -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1923___containerExists := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c "cloud_storage_azure_container" (coalesce nil)) ))) "r") -}}
{{- $_ := (index $_1923___containerExists 0) -}}
{{- $containerExists := (index $_1923___containerExists 1) -}}
{{- $_1924___accountExists := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c "cloud_storage_azure_storage_account" (coalesce nil)) ))) "r") -}}
{{- $_ := (index $_1924___accountExists 0) -}}
{{- $accountExists := (index $_1924___accountExists 1) -}}
{{- $_is_returning = true -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1929_value_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c `cloud_storage_cache_size` (coalesce nil)) ))) "r") -}}
{{- $value := (index $_1929_value_ok 0) -}}
{{- $ok := (index $_1929_value_ok 1) -}}
{{- if (not $ok) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (coalesce nil)) | toJson -}}
{{- break -}}
...
{{- break -}}
{{- end -}}
{{- $size_19 := (get (fromJson (include "redpanda.TieredStorageConfig.CloudStorageCacheSize" (dict "a" (list (deepCopy $c)) ))) "r") -}}
{{- if (ne (toJson $size_19) "null") -}}
{{- $_ := (set $config "cloud_storage_cache_size" ((get (fromJson (include "_shims.resource_Value" (dict "a" (list $size_19) ))) "r") | int64)) -}}
{{- end -}}
{{- $_is_returning = true -}}
