...
func (c *TLSCert) ServerSecretName(state *helmette.Dot, name string) string {
	if c.SecretRef != nil {
...
func (c *TLSCert) ClientSecretName(dot *helmette.Dot, name string) string {
	if c.ClientSecretRef != nil {
		return c.ClientSecretRef.Name
	}
	return fmt.Sprintf("%s-%s-client-cert", Fullname(dot), name)
}

func (c *TLSCert) RootSecretName(dot *helmette.Dot, name string) string {
	return fmt.Sprintf(`%s-%s-root-certificate`, Fullname(dot), name)
}

func (c *TLSCert) CASecretRef(dot *helmette.Dot, name string) corev1.SecretKeySelector {
	// If no SecretRef is specified, we know that the CA was generated by cert-manager.
	if c.SecretRef == nil {
		return corev1.SecretKeySelector{
			LocalObjectReference: corev1.LocalObjectReference{
				Name: c.RootSecretName(dot, name),
			},
	...
	return corev1.SecretKeySelector{
		LocalObjectReference: corev1.LocalObjectReference{
			Name: c.ServerSecretName(dot, name),
		},
		Key: key,
	}
}
...
func (l *ListenerConfig[T]) ConsoleTLS(tls *TLS) ConsoleTLS {
	...
	cert := tls.Certs.MustGet(l.TLS.Cert)
	...
	if tls.Certs.MustGet(l.TLS.Cert).CAEnabled {
		t.CaFilepath = fmt.Sprintf("%s/ca.crt", cert.ServerMountPoint(l.TLS.Cert))
	} else {
		t.CaFilepath = fmt.Sprintf("%s/tls.crt", cert.ServerMountPoint(l.TLS.Cert))
	}
	...
	t.CertFilepath = fmt.Sprintf("%s/tls.crt", cert.ClientMountPoint(l.TLS.Cert))
	t.KeyFilepath = fmt.Sprintf("%s/tls.key", cert.ClientMountPoint(l.TLS.Cert))
