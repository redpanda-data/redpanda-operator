...
func TestValidateGatewayListener(t *testing.T) {
	...
	for _, tag := range []string{"kafka", "http", "admin", "schema"} {
		require.PanicsWithValue(t,
			"external gateway listener "+tag+"/default requires `host` (the bootstrap SNI hostname) when gateway: true",
			func() {
	...
	// A gateway: true listener while the global gateway is not enabled/configured
	// must fail closed (no NodePort/LoadBalancer fallback) — this is the fail-open
	// exposure gap from the re-review.
	require.PanicsWithValue(t,
		"external listener kafka/default sets gateway: true but external.gateway is not enabled with at least one parentRef; refusing to fall back to a NodePort/LoadBalancer Service. Set external.gateway.enabled: true and external.gateway.parentRefs",
		func() {
