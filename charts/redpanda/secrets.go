...
func SecretSTSLifecycle(state *RenderState) *corev1.Secret {
	...
	secret.StringData["common.sh"] = helmette.Join("\n", []string{
		...
		fmt.Sprintf(`CURL_BROKERS_CMD_PREFIX="curl --silent --fail %s ${CURL_URL}/v1/brokers"`, adminCurlFlags),
		``,
	...
	postStartSh := []string{
		...
		`      # compare the advertised address exactly (not as a regex, which would`,
		`      # treat the dots in ${POD_FQDN} as wildcards)`,
		`      BROKER_ADDRESS=$(echo "${BROKER}" | grep -o '\"internal_rpc_address\": *\"[^\"]*' | grep -o '[^\"]*$')`,
		`      if [ "${BROKER_ADDRESS}" != "${POD_FQDN}" ]; then continue; fi`,
		`      # only clear a broker the cluster reports dead and draining: a live`,
		`      # broker at this address is not a ghost, and one that isn't draining`,
		`      # has no maintenance mode to clear`,
		`      if ! echo "${BROKER}" | grep -q '\"is_alive\": *false'; then continue; fi`,
