...
func Secrets(state *RenderState) []*corev1.Secret {
	...
	secrets = append(secrets, SecretConfigurator(state, NamedStatefulset{Statefulset: state.Values.Statefulset}))
	if fsValidator := SecretFSValidator(state, NamedStatefulset{Statefulset: state.Values.Statefulset}); fsValidator != nil {
...
func SecretFSValidator(state *RenderState, ss NamedStatefulset) *corev1.Secret {
...
func SecretConfigurator(state *RenderState, ss NamedStatefulset) *corev1.Secret {
	secret := &corev1.Secret{
		TypeMeta: metav1.TypeMeta{
			APIVersion: "v1",
			Kind:       "Secret",
	...
	kafkaSnippet := secretConfiguratorKafkaConfig(state, ss.Statefulset)
	configuratorSh = append(configuratorSh, kafkaSnippet...)

	httpSnippet := secretConfiguratorHTTPConfig(state, ss.Statefulset)
