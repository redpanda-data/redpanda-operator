...
func Validate(dot *helmette.Dot) {
	values := helmette.Unwrap[Values](dot.Values)

	if values.KubernetesAPIExternalAddress == "" {
		panic("a value must be specified for \"apiServerExternalAddress\"")
	}

	if values.Node == "" {
		panic("a value must be specified for \"node\"")
	}

	if len(values.Peers) < 3 {
		panic("at least 3 nodes must be specified for \"peers\"")
	}

	for i, peer := range values.Peers {
		if peer.Address == "" {
			panic(fmt.Sprintf("peers[%d] must specify an address", i))
		}
		if peer.Name == "" {
			panic(fmt.Sprintf("peers[%d] must specify a name", i))
		}
	}
}
...
func Labels(dot *helmette.Dot) map[string]string {
	...
	return helmette.Merge(labels, SelectorLabels(dot))
}
