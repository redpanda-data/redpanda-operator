...
{{- $dep := (mustMergeOverwrite (dict "metadata" (dict "creationTimestamp" (coalesce nil)) "spec" (dict "selector" (coalesce nil) "template" (dict "metadata" (dict "creationTimestamp" (coalesce nil)) "spec" (dict "containers" (coalesce nil))) "strategy" (dict)) "status" (dict)) (mustMergeOverwrite (dict) (dict "apiVersion" "apps/v1" "kind" "Deployment")) (dict "metadata" (mustMergeOverwrite (dict "creationTimestamp" (coalesce nil)) (dict "name" (get (fromJson (include "multicluster.Fullname" (dict "a" (list $dot)))) "r") "labels" (get (fromJson (include "multicluster.Labels" (dict "a" (list $dot)))) "r") "namespace" $dot.Release.Namespace)) "spec" (mustMergeOverwrite (dict "selector" (coalesce nil) "template" (dict "metadata" (dict "creationTimestamp" (coalesce nil)) "spec" (dict "containers" (coalesce nil))) "strategy" (dict)) (dict "replicas" ((1 | int) | int) "selector" (mustMergeOverwrite (dict) (dict "matchLabels" (get (fromJson (include "multicluster.SelectorLabels" (dict "a" (list $dot)))) "r"))) "template" (get (fromJson (include "multicluster.StrategicMergePatch" (dict "a" (list (mustMergeOverwrite (dict "metadata" (dict "creationTimestamp" (coalesce nil)) "spec" (dict "containers" (coalesce nil))) (dict "metadata" (mustMergeOverwrite (dict "creationTimestamp" (coalesce nil)) (dict "labels" $values.podTemplate.metadata.labels "annotations" $values.podTemplate.metadata.annotations)) "spec" $values.podTemplate.spec)) (mustMergeOverwrite (dict "metadata" (dict "creationTimestamp" (coalesce nil)) "spec" (dict "containers" (coalesce nil))) (dict "metadata" (mustMergeOverwrite (dict "creationTimestamp" (coalesce nil)) (dict "labels" (get (fromJson (include "multicluster.SelectorLabels" (dict "a" (list $dot)))) "r"))) "spec" (mustMergeOverwrite (dict "containers" (coalesce nil)) (dict "serviceAccountName" (get (fromJson (include "multicluster.ServiceAccountName" (dict "a" (list $dot)))) "r") "volumes" (get (fromJson (include "multicluster.operatorPodVolumes" (dict "a" (list $dot)))) "r") "containers" (get (fromJson (include "multicluster.operatorContainers" (dict "a" (list $dot (coalesce nil))))) "r"))))))))) "r"))))) -}}
{{- $_is_returning = true -}}
{{- (dict "r" $dep) | toJson -}}
{{- break -}}
{{- end -}}
