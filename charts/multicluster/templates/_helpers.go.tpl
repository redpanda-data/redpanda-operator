...
{{- /* Transpiled by gotohelm from "github.com/redpanda-data/redpanda-operator/charts/multicluster/v25/helpers.go" */ -}}

{{- define "multicluster.Validate" -}}
{{- $dot := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $values := $dot.Values.AsMap -}}
{{- if (eq $values.apiServerExternalAddress "") -}}
{{- $_ := (fail "a value must be specified for \"apiServerExternalAddress\"") -}}
{{- end -}}
{{- if (eq $values.node "") -}}
{{- $_ := (fail "a value must be specified for \"node\"") -}}
{{- end -}}
{{- if (lt ((get (fromJson (include "_shims.len" (dict "a" (list $values.peers)))) "r") | int) (3 | int)) -}}
{{- $_ := (fail "at least 3 nodes must be specified for \"peers\"") -}}
{{- end -}}
{{- range $i, $peer := $values.peers -}}
{{- if (eq $peer.address "") -}}
{{- $_ := (fail (printf "peers[%d] must specify an address" $i)) -}}
{{- end -}}
{{- if (eq $peer.name "") -}}
{{- $_ := (fail (printf "peers[%d] must specify a name" $i)) -}}
{{- end -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
...
{{- define "multicluster.Name" -}}
{{- $dot := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (get (fromJson (include "multicluster.cleanForK8s" (dict "a" (list $dot.Chart.Name)))) "r")) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}

{{- define "multicluster.Fullname" -}}
{{- $dot := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $name := (get (fromJson (include "multicluster.Name" (dict "a" (list $dot)))) "r") -}}
{{- if (contains $name $dot.Release.Name) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (get (fromJson (include "multicluster.cleanForK8s" (dict "a" (list $dot.Release.Name)))) "r")) | toJson -}}
{{- break -}}
{{- end -}}
{{- $_is_returning = true -}}
{{- (dict "r" (get (fromJson (include "multicluster.cleanForK8s" (dict "a" (list (printf "%s-%s" $dot.Release.Name $name))))) "r")) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}

{{- define "multicluster.ChartName" -}}
{{- $dot := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $chart := (printf "%s-%s" $dot.Chart.Name $dot.Chart.Version) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (get (fromJson (include "multicluster.cleanForK8s" (dict "a" (list (replace "+" "_" $chart))))) "r")) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}

{{- define "multicluster.Labels" -}}
{{- $dot := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
...
{{- end -}}
{{- $_is_returning = true -}}
{{- (dict "r" (merge (dict) $labels (get (fromJson (include "multicluster.SelectorLabels" (dict "a" (list $dot)))) "r"))) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}

{{- define "multicluster.SelectorLabels" -}}
{{- $dot := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (dict "app.kubernetes.io/name" (get (fromJson (include "multicluster.Name" (dict "a" (list $dot)))) "r") "app.kubernetes.io/instance" $dot.Release.Name)) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}

{{- define "multicluster.cleanForK8s" -}}
{{- $s := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
...
{{- define "multicluster.cleanForK8sWithSuffix" -}}
{{- $s := (index .a 0) -}}
{{- $suffix := (index .a 1) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
...
{{- define "multicluster.StrategicMergePatch" -}}
{{- $overrides := (index .a 0) -}}
{{- $original := (index .a 1) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
...
{{- $_183_overrideVol_1_ok_2 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $overrideVolumes $vol.name (dict "name" ""))))) "r") -}}
{{- $overrideVol_1 := (index $_183_overrideVol_1_ok_2 0) -}}
{{- $ok_2 := (index $_183_overrideVol_1_ok_2 1) -}}
{{- if $ok_2 -}}
...
{{- $_322_override_3_ok_4 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $overrideContainers $container.name (coalesce nil))))) "r") -}}
{{- $override_3 := (index $_322_override_3_ok_4 0) -}}
{{- $ok_4 := (index $_322_override_3_ok_4 1) -}}
{{- if $ok_4 -}}
...
{{- $_353_override_5_ok_6 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $overrideContainers $container.name (coalesce nil))))) "r") -}}
{{- $override_5 := (index $_353_override_5_ok_6 0) -}}
{{- $ok_6 := (index $_353_override_5_ok_6 1) -}}
{{- if $ok_6 -}}
...
{{- $_384_override_7_ok_8 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $overrideEphemeralContainers $container.name (coalesce nil))))) "r") -}}
{{- $override_7 := (index $_384_override_7_ok_8 0) -}}
{{- $ok_8 := (index $_384_override_7_ok_8 1) -}}
{{- if $ok_8 -}}
