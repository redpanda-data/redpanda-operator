...
5. **Check changie replacement coverage.** The `.changie.yaml` `replacements` config auto-updates some files but not all. After `changie merge`:
   - **`operator` project**: Auto-updates `operator/chart/Chart.yaml` (version, appVersion, image tag) and golden file version labels. However, the `helm.sh/chart` label in golden files uses the version *without* a `v` prefix and the changie regex expects one — so golden files need regenerating via tests (step 6).
   - **`charts/redpanda` project**: Has **no** changie replacements configured. You must manually bump `version:` in `charts/redpanda/Chart.yaml`.
   - **README.md badges**: Both `operator/chart/README.md` and `charts/redpanda/README.md` contain version badges that are regenerated by `task generate` in CI. Update these manually to match the new versions.
