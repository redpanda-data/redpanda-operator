...
- Add the controller to the `controller-gen` RBAC generation loop in `taskfiles/k8s.yml` (search for the `for:` block with `NAME`/`PATH` entries). This generates `operator/config/rbac/itemized/<resource>.yaml` from kubebuilder RBAC markers. Also add the itemized file to the chart copy step (search for `chart/files/rbac/`) so it gets synced to `operator/chart/files/rbac/<resource>.ClusterRole.yaml`.
