...
Regenerate goldens with `-update-golden`: `go test ./path/to/... -update-golden`. On this branch `pkg/testutil` delegates to `common-go/goldenfile` and honors either flag, so the legacy `-update` also works for suites using it (e.g. chart `TestTemplate`); prefer `-update-golden`.
