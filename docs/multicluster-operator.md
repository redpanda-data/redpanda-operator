...
- The `StretchCluster` spec consistency check records a `ClusterUnreachable` condition and continues reconciling on all reachable clusters. Reconciliation is **not** blocked.
- All reconciliation operations targeting the unreachable cluster — resource provisioning, StatefulSet/pod updates, rolling restarts, resource sync, and deletion cleanup — are skipped gracefully with a log message. They are retried on the next reconcile pass once the cluster is reachable again. Operations on all other clusters proceed normally.
...
- During `StretchCluster` deletion, cleanup on the unreachable cluster is deferred: the operator records that deletion is incomplete and requeues rather than failing. Once the cluster reconnects, cleanup resumes automatically.
