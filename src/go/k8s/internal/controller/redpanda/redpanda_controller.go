...
func (r *RedpandaReconciler) tryMigration(ctx context.Context, log logr.Logger, rp *v1alpha1.Redpanda) error {
	...
	if err != nil {
		...
		log.V(logger.DebugLevel).Info(msg, "cluster-name", annotatedCluster.Name, "annotations", annotatedCluster.Annotations)
	...
	if err != nil {
		...
		log.V(logger.DebugLevel).Info(msg, "console-name", annotatedConsole.Name, "annotations", annotatedConsole.Annotations)
	...
	if err != nil {
		...
		log.V(logger.DebugLevel).Info(msg, "service-name", internalService.Name, "labels", internalService.Labels, "annotations", internalService.Annotations)
	...
	if ptr.Deref(rp.Spec.ClusterSpec.Console.Enabled, true) {
		...
		if err != nil {
			...
			log.V(logger.DebugLevel).Info(msg, "service-name", annotatedConsoleSVC.Name, "labels", annotatedConsoleSVC.Labels, "annotations", annotatedConsoleSVC.Annotations)
...
func hasLabelsAndAnnotations(object client.Object, rp *v1alpha1.Redpanda) bool {
	return object.GetLabels()["app.kubernetes.io/managed-by"] == helm && object.GetAnnotations()["meta.helm.sh/release-name"] == rp.Name && object.GetAnnotations()["meta.helm.sh/release-namespace"] == rp.Namespace
}
