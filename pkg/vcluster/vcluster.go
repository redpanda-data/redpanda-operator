...
func ForTestInShared(t *testing.T) *Cluster {
	...
	if deadline, ok := t.Deadline(); ok {
		// Calculate a safe margin before the global deadline
		// For example, 1 second before the test will be forcibly killed
		safeDeadline := deadline.Add(-1 * time.Second)

		// Create a new context with this calculated deadline
		var cancel context.CancelFunc
		ctx, cancel = context.WithDeadline(ctx, safeDeadline)
		defer cancel()
	}

	cluster, err := NewInShared(ctx)
