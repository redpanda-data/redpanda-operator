...
func TestRender(t *testing.T) {
	...
	goldenPools := goldenfile.NewTxTar(t, "testdata/render-cases.pools.golden.txtar")
	goldenResources := goldenfile.NewTxTar(t, "testdata/render-cases.resources.golden.txtar")
	...
	for _, file := range casesArchive.Files {
		t.Run(file.Name, func(t *testing.T) {
			...
			sets, err := RenderBrokerPools(state)
			require.NoError(t, err)

			poolBytes, err := yaml.Marshal(sets)
			require.NoError(t, err)
			goldenPools.AssertGolden(t, goldenfile.YAML, file.Name, poolBytes)
			...
			require.NoError(t, err)
			goldenResources.AssertGolden(t, goldenfile.YAML, file.Name, resourceBytes)
