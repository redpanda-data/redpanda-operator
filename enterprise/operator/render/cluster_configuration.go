...
func clusterConfigurationToConfiguration(cc redpandav1alpha2.ClusterConfiguration) (map[string]string, []Fixup, []corev1.EnvVar) {
	if len(cc) == 0 {
		return nil, nil, nil
	}
	...
	for _, k := range sortedMapKeys(cc) {
		...
		case v.ExternalSecretRefSelector != nil:
			...
			cel := fmt.Sprintf(`%s("%s")`, CELExternalSecretRef, v.ExternalSecretRefSelector.Name)
			if !v.UseRawValue {
				cel = fmt.Sprintf(`%s(%s)`, CELRepr, cel)
			}
			if ptr.Deref(v.ExternalSecretRefSelector.Optional, false) {
				cel = fmt.Sprintf(`%s(%s)`, CELErrorToWarning, cel)
			}
			fixups = append(fixups, Fixup{Field: k, CEL: cel})
		}
	}
...
func envVarFixup(key string, source *corev1.EnvVarSource, useRawValue bool) (corev1.EnvVar, Fixup) {
	envName := keyToEnvVar(key)

	cel := fmt.Sprintf(`%s("%s")`, CELEnvString, envName)
	if !useRawValue {
		cel = fmt.Sprintf(`%s(%s)`, CELRepr, cel)
	}

	return corev1.EnvVar{Name: envName, ValueFrom: source},
		Fixup{Field: key, CEL: cel}
}
