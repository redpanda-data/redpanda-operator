...
func tieredStorageToConfiguration(storage *redpandav1alpha2.StretchStorage) (map[string]any, []Fixup, []corev1.EnvVar) {
...
func credFixup(field, envName string, ref *redpandav1alpha2.SecretWithConfigField) (corev1.EnvVar, Fixup) {
	return corev1.EnvVar{Name: envName, ValueFrom: credSecretRefSource(ref)},
		Fixup{Field: field, CEL: fmt.Sprintf(`repr(envString("%s"))`, envName)}
}
