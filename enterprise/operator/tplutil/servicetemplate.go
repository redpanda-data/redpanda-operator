...
// ServiceOverrides represents a set of Service overrides using Kubernetes
...
// ServiceStrategicMergePatch applies Service overrides to an existing Service
// using a strategic-merge-patch-like approach. Ports are merged by name key
// rather than replaced wholesale.
func ServiceStrategicMergePatch(overrides ServiceOverrides, original corev1.Service) (corev1.Service, error) {
	...
	var err error
	overrides, err = MergeTo[ServiceOverrides](cloned)
	if err != nil {
		return zero, err
	}
	...
	merged, err := MergeTo[corev1.Service](
	...
	merged.Spec.Ports = MergeSliceBy(
		original.Spec.Ports,
		overrideSpec.Ports,
		"name",
		mergeServicePort,
...
func mergeServicePort(original corev1.ServicePort, override applycorev1.ServicePortApplyConfiguration) corev1.ServicePort {
	result, _ := MergeTo[corev1.ServicePort](override, original)
	return result
}
